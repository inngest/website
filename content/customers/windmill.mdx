---
title: How Windmill's AI Agent helps manage thousands of employees using Inngest
companyName: Windmill
logo: /assets/customers/windmill-logo.svg
companyEmployees: 2-20
companyIndustry: SaaS / AI Agents
companyUseCase: AI Agents
quote:
  text: >-
    LLMs APIs are pretty different than other traditional APIs, which are
    usually pretty fast and reliable; they are slow and pretty brittle.
...
companyURL: 'https://gowindmill.com'
companyDescription: Your AI Manager Assistant
ogImage: /assets/customers/windmill/cover.jpg
---
...
By using Inngest's [retries](/docs/features/inngest-functions/error-retries/retries?ref=case-study-windmill), [rate limit control](/docs/guides/rate-limiting?ref=case-study-windmill), and [batching](/docs/guides/batching?ref=case-study-windmill) features, their integrations are guaranteed to run reliably, without data loss or database outage:
...
<video
  ...
  poster="/assets/customers/windmill/cover.jpg"
  playsInline
  loop
  controls
  preload="metadata"
...
This conversational UX pattern was made possible by relying on Inngest's [priority queues](/docs/guides/priority?ref=case-study-windmill), as Max explains:
...
- [**Durability**](/customers/baerskin-tactical?ref=case-study-windmill): Inngest's steps split each workflow into retriable steps, gracefully handling any 3rd party or LLM APIs failures.
- [**Multitenancy and Flow control**](/docs/guides/flow-control?ref=case-study-windmill): Evenly spreading the execution of reporting workflows, to avoid scenarios when a single user asking multiple reports slows down the global user experience.
- [**Observability**](/docs/platform/monitor/observability-metrics?ref=case-study-windmill): Each workflow's step becomes introspectable with access to its input and output, along with metrics to quickly evaluate the overall user experience.
...
If you're looking for a better way to orchestrate workflows, manage concurrency, and scale AI-powered applications, [**book a demo**](/contact?ref=case-study-windmill) today and see how Inngest can streamline your infrastructure.
