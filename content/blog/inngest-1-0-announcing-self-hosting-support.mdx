...
**Postgres support is now available** (as of January 2025) as an alternative to SQLite for production deployments. Use the `--postgres-uri` flag with the Inngest CLI (v1.4.0+) to connect to your Postgres database for enhanced scalability and production-grade persistence.
```bash
inngest start --postgres-uri="postgresql://user:password@localhost:5432/inngest"
```

This makes it easier for teams already using Postgres to adopt Inngest self-hosting with their existing database infrastructure.
...
* **Postgres database persistence support** (January 2025) - You can now use your own Postgres database for persistence instead of the default bundled SQLite implementation. See the [self-hosting docs](/docs/self-hosting) for configuration details.
