...
We built Inngest as a reliability layer for modern applications. It enables developers to write declarative  step functions in code by combining durable execution, event streams, and queues into a single serverless platform.
...
To set context, let's walk through how Inngest uses a queue, with an example function configuration using our [TS SDK](/docs/reference/typescript/intro) ([Go](https://pkg.go.dev/github.com/inngest/inngestgo) and [Python](/docs/reference/python) SDKs are also available, with more coming soon):
...
- Once a function runs a step, we store the step's result inside the function run's state and (worst case) enqueue *another* job for the next step. There's some nuance here for long-running servers, though we'll talk about that in an execution deep dive separately.
...
If you're putting that fairness to work across many customer-facing SaaS syncs, see [Have lots of SaaS integrations? You don't need lots of reliability models](/blog/have-lots-of-saas-integrations-you-dont-need-lots-of-reliability-models?ref=blog-building-the-inngest-queue-pt-i-fairness-multi-tenancy).
