...
Traditional durable execution implementations face a fundamental tradeoff: **durability comes at the cost of latency**. Each step requires synchronous communication with the execution engine to persist state, introducing inter-step delays of tens to hundreds of milliseconds.

For background workflows—batch jobs, scheduled tasks—this latency is acceptable. However, modern applications increasingly require durable execution for **user-facing, real-time workloads** where every millisecond matters: interactive AI chat applications, real-time data processing, and live dashboards.
