...
#### Checkpointing: Optimizing state persistence for latency

Traditional state persistence requires synchronous communication with the execution engine to persist state before proceeding, introducing inter-step delays of tens to hundreds of milliseconds. For background workflows—batch jobs, scheduled tasks—this latency is acceptable. However, modern applications increasingly require durable execution for **user-facing, real-time workloads** where every millisecond matters: interactive AI chat applications, real-time data processing, and live dashboards.

**Checkpointing** addresses this by decoupling execution from persistence. Steps execute immediately while state is checkpointed asynchronously in the background. On failure, workflows resume from the last checkpoint, reducing inter-step latency from hundreds of milliseconds to single-digit milliseconds.

The checkpoint interval determines how much work might need to be replayed after a failure. This is acceptable for latency-sensitive applications with fast, idempotent steps, but traditional synchronous persistence remains better for background batch processing or workflows with expensive side effects that cannot safely replay.
...
* **Schedules the function:** Assigns the function to a worker. [Learn more about serving functions](/docs/learn/serving-inngest-functions)
* **Executes the function:** Runs the function and handles any exceptions. [See how functions are executed](/docs/learn/how-functions-are-executed)
* **Manages state:** Stores intermediate results and ensures consistency. For latency-sensitive workloads, checkpointing enables near-zero inter-step latency while maintaining durability. [Learn about checkpointing](/docs/setup/checkpointing)
* **Retries failures:** Automatically retries failed functions with exponential backoff. [Read about error handling](/docs/guides/error-handling)
* **Scales dynamically:** Adjusts resources to handle varying workloads. [Explore the platform](/docs/platform)

**A simple example** (reference: [Inngest documentation](/docs/learn/inngest-functions?ref=blog-principles-of-durable-execution))
...
2. **Install the SDK:** Follow the [documentation](/docs?ref=blog-principles-of-durable-execution) to install the SDK in your preferred language
