...
AI agents are rapidly becoming the backbone of modern applications. Powering everything from natural language interfaces to complex backend orchestration. But building reliable, multi-step agents still requires solving challenges around state, latency, and context.

At Inngest, we’ve been exploring how to simplify agent orchestration with [**AgentKit**](https://agentkit.inngest.com/overview) a developer-first framework for building agentic workflows using code, not configuration.

To put AgentKit to the test, we built a multi-agent system that queries and analyzes **3.5 million powerlifting records** using natural language.
...
## Streaming, State Management & Race Conditions

AgentKit provides WebSocket-based real-time streaming out of the box. Initially, we streamed final responses directly to the UI and persisted chat history in MySQL.

This introduced issues:
- The frontend depended on the DB for the initial chat load
- If the WebSocket closed prematurely (e.g. during inference), the UI could get stuck in an inconsistent state

To resolve this, we migrated to **Convex** as our reactive database layer. Now, the agent writes once to the DB, and Convex pushes updates directly to the frontend ensuring a single source of truth and eliminating race conditions.
...
We defined a dedicated agent the **Meet performance analyst** responsible for converting natural language into structured tool calls. This agent exclusively invoked the `get_meet_results()` function, which abstracts over a SQL query targeting a ClickHouse database.
