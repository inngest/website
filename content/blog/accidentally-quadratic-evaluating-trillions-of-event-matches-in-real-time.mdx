...
This is the core of the challenge that we have with Inngest. Inngest allows developers to create event-driven functions that can wait for additional events that match specific [filter expressions](/docs/reference/typescript/v4/functions/step-wait-for-event#advanced-event-matching-with-if), or declare that functions should automatically be [be cancelled by specific events](/docs/guides/cancel-running-functions). The hard part is abstracted away from the developer with a line of code.
...
These are everywhere in Inngest. As we highlighted, they come in the form of [waitForEvent](/docs/reference/typescript/v4/functions/step-wait-for-event#step-wait-for-event-id-options-promise-null-event-payload) and [cancelOn](/docs/reference/typescript/functions/cancel-on) expressions. Similar functionality powers our [invoke](/docs/reference/typescript/v4/functions/step-invoke) functionality to execute child functions, but we'll focus on the former two.
