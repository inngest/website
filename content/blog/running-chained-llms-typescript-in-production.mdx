...
- Run each step in series or parallel, injecting the current function state from [`step.run`](/docs/reference/typescript/v4/functions/step-run) blocks into the appropriate variables.
...
- [Handle failures](/docs/features/inngest-functions/error-retries/inngest-errors?ref=blog-chained-llms), allowing you to create cleanup code or notify users on error.
