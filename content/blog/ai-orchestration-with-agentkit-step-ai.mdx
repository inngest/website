...
## Stream AgentKit workflows to the frontend with useAgent

**Released September 2025:** Building on AgentKit's orchestration capabilities, we've released **useAgent**—a React hook that makes it trivial to stream real-time updates from your durable AI workflows directly to the browser.
...
- **Built-in durability**: Refresh mid-execution and resume exactly where you left off
- **No infrastructure**: No WebSocket management, state stores, or connection handling required
```tsx
  ...
  return (
    <div>
      {messages.map(({ id, role, parts }) => (
        <div key={id}>
          {parts.map(({ type, content }) => 
            type === "text" ? <p>{content}</p> : null
          )}
        </div>
      ))}
...
[Read the full blog post](/blog/agentkit-useagent-realtime-hook) | [Explore the documentation](https://agentkit.inngest.com/streaming/overview) | [View changelog](https://www.inngest.com/changelog/2025-09-24-agentkit-use-agent)
...
- **Query AI metrics with Insights**: Use SQL to analyze token usage, model performance, and agent success rates across your AI workflows

For AI applications, Insights is particularly powerful—query token costs across models, identify which agents loop most frequently, and track success rates over time. All queryable with SQL directly in the dashboard. [Learn more about Insights](/blog/insights-query-events-and-runs)
