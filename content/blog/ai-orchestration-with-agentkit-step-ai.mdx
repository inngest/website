...
Building on AgentKit's orchestration capabilities, we've released **useAgent**—a React hook that makes it trivial to stream real-time updates from your durable AI workflows directly to the browser.
...
[Read the full blog post](/blog/agentkit-useagent-realtime-hook) or explore the [useAgent documentation](https://agentkit.inngest.com/use-agent).
...
For AI applications, Insights is particularly powerful—query token costs across models, identify which agents loop most frequently, and track success rates over time. All queryable with SQL directly in the dashboard.

[Learn more about Insights](/blog/insights-query-events-and-runs)
