...
what matters. But LLMs have hard context window limits, and when you exceed them, the model silently drops information (usually from the beginning or
middle of context). You don't control what gets dropped unless you prioritize what goes in. When this happens mid-task, the agent doesn’t gracefully degrade. It **hallucinates or fails silently.**
...
- Budget your context window and measure usage to prevent hitting the limit mid-reasoning:
...
3. **Set temperature=0** unless you need creativity
...
**Quick Tip: Make your loops replayable**. When something break

```tsx
// In Inngest dashboard:
// Click "Replay Run" → select failed step → see exact context
// Change one variable → rerun from that step → validate fix
```
