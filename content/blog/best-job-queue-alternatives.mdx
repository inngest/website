...
Look past Celery when the pain is state across steps, fairness, or actually seeing a run. Same list as the Node teams, just with Beat and Flower standing in for Bull Board. [Inngest's Python SDK](/docs/getting-started/python-quick-start?ref=blog-best-job-queue-alternatives) covers that shape. So do Temporal and Restate. None of them is a drop-in replacement for `delay()`. You're changing how you write the code, not swapping a library.
