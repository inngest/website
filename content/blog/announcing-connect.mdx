...
While Inngest has been primarily successful among Serverless developers, we've seen a growing demand from developers running Inngest Functions on their long-running servers such as Render, AWS EC2, DigitalOcean and such.
...
[Connect](/docs/setup/connect) is a drop-in replacement to [`serve()`](/docs/learn/serving-inngest-functions) to connect your application to Inngest over one or multiple outbound persistent connections, **dramatically improving the latency and scalability of your functions**.
...
Beyond huge performance and scalability benefit, Connect also brings a better developer experience, **enabling you to colocate all your Inngest-related code in a single file**:
...
Once your application is deployed using Connect, each active connection get listed as a worker in the Inngest Platform, bringing you full visibility
