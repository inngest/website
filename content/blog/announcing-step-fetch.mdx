...
Inngest's `fetch()` utility can be passed as a custom fetch handler to make all axios requests durable:
...
The `fetch()` utility is context-aware and will fall back to the native `fetch()` API if it is not running within an Inngest function:
