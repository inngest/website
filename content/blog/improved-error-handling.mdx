...
We built Inngest to help you build reliable products. Every Inngest function has automatic retries. Functions can be broken down into individual steps which are all individually executed and [retried on error](/docs/features/inngest-functions/error-retries/inngest-errors). This makes your code durable. Inngest acts like a nice reliability layer on top of your code.

Let's see how it's done. Inngest wraps your code within a [`step.run()`](/docs/reference/typescript/v4/functions/step-run) to capture these errors and retry until attempts are exhausted, which marks that step as “failed.” With automatic retries, Inngest catches the error for you and retries the step, but sometimes errors are expected and you can, or _need_, to handle them gracefully.
...
This change is available today in our [TypeScript](/docs/reference/typescript/intro) (`v3.12.0`) and [Go](https://pkg.go.dev/github.com/inngest/inngestgo) (`v0.6.0`) SDKs. We will be rolling this out to the Python SDK in the coming days. You'll also need the latest version Inngest Dev Server ([`v0.25.0`](https://github.com/inngest/inngest)) which you can run using `@latest`, for example: `npx --ignore-scripts=false inngest-cli@latest dev`.
