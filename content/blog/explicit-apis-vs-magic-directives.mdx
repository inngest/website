...
We know—because we had to make the same decision when we built Inngest. 

Let's explore what we learned, why we shifted our approach *away* from Vercel’s current model—and why that matters for production applications.

## Core Differences Between Inngest and Vercel’s Workflow Development Kit
...
**Inngest** is a compute platform-agnostic orchestration platform. It’s built around events triggering functions, with a queue managing execution. You write step functions (`"step.run"`) that explicitly define steps with IDs, while Inngest handles coordination across concurrent runs with built-in flow control, observability, and recovery tools.
...
- **Coordination.** Production agents don't run in isolation—you may have thousands executing simultaneously. Inngest handles this out of the box (queuing, backpressure, per-tenant limits). Vercel focuses on individual function durability; you'd build coordination yourself.
- **Multi-tenancy.** If Customer A triggers 10K agent runs, can you prevent it from degrading Customer B's experience? Inngest has native primitives for this. Vercel doesn't address it.
...
- **Event-driven vs RPC.** Inngest's event model naturally handles fan-out (one event → many functions), replay, and service decoupling. Vercel is function-call based. Different mental models.
...
- **Speed to first workflow.** Inngest’s event model may be easier to iterate on, but directives are faster to learn for someone new to this work. Add `"use workflow"` and your function is durable. No conceptual overhead.
...
Vercel's WDK looks elegant in examples. `"use workflow"` is simpler than `step.run("step-id", ...)` in documentation. We tried that approach. Unfortunately, we know it often breaks in unexpected (and unobservable) ways in production use cases.
