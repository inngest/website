...
* [**Flow control**](/docs/guides/flow-control?ref=blog-neon-integration) \- Combine database triggers with flow control functionality like throttling, debouncing, rate limiting, or singleton execution for better resource management and efficiency. For example, use throttling for working with third party API rate limits, use debounce for operations that may happen frequently, helping to avoid redundant work, or use Singleton Functions to ensure only one sync operation runs per user at a time.
