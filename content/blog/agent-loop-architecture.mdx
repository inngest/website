...
Most people think about agents as "LLM + tools." The agent loop architecture re-frames this as agents being "loops + skills + orchestration." The LLM + tools are inside the loops. LLMs and tools [can be swapped or tweaked while the architecture remains](/blog/your-agent-architecture-has-a-half-life?ref=blog-agent-loop-architecture). The orchestration enables the architecture.
