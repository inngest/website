...
Each context-gathering operation becomes a [durable step](/blog/ai-agents-inngest-durable-steps?ref=blog-five-lessons-for-context-engineering) in Inngest. When cubic's agent calls a terminal command, reads a file, or performs a git operation, that work is wrapped in step.run() with automatic retries and failure handling.
