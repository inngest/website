...
## How Inngest has evolved: New execution modes (Jan 2026)

### Connect: Persistent connection mode
For container deployments (Kubernetes, ECS, etc.), Inngest now supports **Connect**—outbound persistent connections that eliminate the need for load balancers and reduce latency:
- Lower latency through persistent connections
- Elastic horizontal scaling by adding more workers
- Simpler long-running steps not bound by HTTP timeouts

[Learn more about Connect](/docs/setup/connect)

### Singleton Functions: Exclusive execution
Control workflow execution with **Singleton Functions** to ensure only one instance of a function runs at a time:
- `skip` mode: Discard new events while a run is in progress
- `cancel` mode: Cancel the existing run and start fresh with the latest data

Perfect for scenarios where you want to process only the most recent data.

[Learn more about Singleton Functions](/docs/guides/singleton)

### step.fetch(): Durable HTTP requests
Make HTTP requests durable by offloading them to the Inngest platform:
- Automatic retries on failure
- Requests execute outside your serverless timeout limits
- Built-in observability for all external API calls

[Learn more about step.fetch()](/docs/features/inngest-functions/steps-workflows/step-fetch)

These additions expand how workflow engines can be used in production systems.
