...
Asynchronous work is a huge part of any application. It includes a great deal: running long running tasks as background jobs, scheduling work with cron jobs, data processing pipelines, [change data capture](https://en.wikipedia.org/wiki/Change_data_capture), and [handling webhooks in a performant and resilient manner](/blog/building-webhooks-that-scale). Modern webhook handling requires flexibility—Inngest now supports multiple content types and programmatic webhook management. Often you need to combine all of these with database state to create complex, stateful user workflows. For example, an app may communicate with an external service, then will need to follow up with another action X hours or days later and perform another action depending on new data received.
