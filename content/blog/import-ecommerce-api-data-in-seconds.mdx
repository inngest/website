...
export default inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    while (hasMore) {
      ...
      allProducts.push(...page.data.products.nodes)
...
You can combine these step tools in any way that you want. For example, you may prefer to batch your database inserts or you may want an additional step for each product that fetches and imports [all product images](https://shopify.dev/docs/api/admin-graphql/latest/objects/product#field-Product.fields.media) into your database or copies them to your image storage bucket.

## Using fan-out for faster parallel processing

We've learned that you can use [`step.run()`](/docs/learn/inngest-steps?ref=blog-import-ecommerce) to break your job into reliable steps, but what if you want to have different levels of concurrency for part of your code? You can send events from one function to “fan-out” work to another function and run that work in parallel, speeding up your pipeline.
...
inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    for (let product of allProducts) {
      ...
      const imagesData = await step.run("fetch-images-product", async () => {
        return await client.query({
          data: `query GetImages {
      ...
      const events = imagesData.data.product.media.nodes.map((image) => ({
