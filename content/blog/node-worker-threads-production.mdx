...
In a [previous post](/blog/node-worker-threads?ref=blog-node-worker-threads-production), I wrote about why we moved [Inngest Connect](/docs/setup/connect?ref=blog-node-worker-threads-production) internals into a Node.js worker thread.
...
But the first version of that sentence hides most of the work.

```ts
new Worker("./runner.js");
```

That line isn't the hard part. The hard part is everything around it: deciding what crosses the thread boundary, designing the message protocol, making logs work, packaging the worker file, respawning after crashes, exposing debug state, and shutting down without dropping in-flight work.
...
If you want the TL;DR before reading the full walkthrough, skip to the [production checklist](#production-checklist).
...
## Build a protocol, not a pile of messages
...
## Callbacks become request-response
...
## Make startup work in every runtime you support
...
## Expose worker health to the parent
...
If you're using worker threads for production infrastructure, make sure you've handled the lifecycle around the thread:

- Define the thread boundary.
- Design typed messages for both directions.
- Keep behaviorful objects on the parent thread.
- Add correlation IDs, timeouts, and cleanup for in-flight request-response messages.
- Handle worker crash and restart with capped backoff.
- Send worker health, connection state, and in-flight counts back to the parent thread.
- Package the worker file explicitly.
- Treat shutdown as a drain protocol.
- Test crash, reconnect, shutdown, and late-response cases.
