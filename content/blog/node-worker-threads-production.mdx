...
## Mirror observability back to the parent

Once the connection manager lives in a worker, the main thread no longer has direct access to its state.

That matters for debugging. Users still expect to inspect the connection. The parent still needs to know whether the connection is active, closing, reconnecting, or closed. It needs in-flight request counts, heartbeat timestamps, and shutdown state.

The worker periodically sends debug state to the main thread:
