...
If you’re familiar with both Astro and Inngest, you can read about [how to set up the Inngest API in Astro](/docs/learn/serving-inngest-functions#framework-astro) or explore [the Inngest and Astro 'Hello World' example](https://github.com/inngest/inngest-js/tree/main/examples/framework-astro).
...
Instead, a single engineer, product manager, or even indie hacker can use Inngest to handle the execution but also see the [error logs](/docs/guides/logging), have the [failed tasks be automatically retried](/docs/features/inngest-functions/error-retries/inngest-errors), and obtain useful metadata.
...
In other words, a function owned by the product team can live in the Astro site and other functions in your backend codebase. At the moment, Inngest has three SDKs available ([TypeScript/JavaScript](/docs/reference/typescript/intro), [Python](/docs/reference/python), [Go](https://pkg.go.dev/github.com/inngest/inngestgo)) but others are planned as well.

## Get started now

If you'd like to learn more about Inngest, head to our [Quick Start Guide](/docs/getting-started/nextjs-quick-start) for a guided tour around the [Inngest Dev Server](/docs/local-development#inngest-dev-server).
