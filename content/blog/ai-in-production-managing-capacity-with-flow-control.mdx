...
We can fix that using chaining with durable execution. Every individual API call can become a separate run in a [multi-step process](https://www.inngest.com/docs/learn/inngest-steps?ref=blog-manage-capacity-ai). Using multi-step functions, you can:
...
### Singleton Functions for exclusive execution

For cases where you need to ensure only one run executes at a time per user or resource, use Singleton Functions (released June 2025):
...
This ensures if a user triggers multiple AI generations rapidly, only the latest one executes—perfect for expensive AI operations.

[Learn more](/docs/guides/singleton)
