...
**Key Insight:** Although run state as a whole is mutable, many of the highest-read pieces of that state are immutable once written. In particular, triggering events and step inputs/outputs do not change, which made aggressive caching possible without complex invalidation.
...
The proxy is a standard Kubernetes Deployment sitting between all services that interact with the [state store](/product/how-inngest-works?ref=blog-redis-stateful-caching) and Redis. Every read and write goes through it.

The routing logic is critical. A given run's state needs to consistently land on the same proxy node, because that's where its cache lives. We achieved this with consistent hashing based on run ID at the gRPC layer, using a headless Kubernetes service to expose pod IPs directly to clients.
...
The main question for the cache layer was simple: what can we cache safely? Once we mapped the data's [mutability boundaries](/blog/how-durable-workflow-engines-work?ref=blog-redis-stateful-caching), the answer was fairly straightforward.

Events that trigger a function run are written once and never change. Step inputs and outputs are written once per step and never change. These can be cached freely and never need invalidation. They live on the proxy node's local cache until the node is recycled.

The execution stack and run metadata are mutable. They change with each step execution. We route reads for these directly to Redis. We considered caching the stack with validation against metadata, but the stack is capped at 1000 entries and fetching it directly from Redis proved cheap enough in practice. The added complexity wasn't really worth it.
...
We benchmarked both options and they were close. Performance-wise, either would have worked. The difference showed up in allocations per operation. PebbleDB made significantly fewer allocations, which meant less pressure on Go's garbage collector and a more predictable memory footprint under load.

Configuration flexibility also helped. We disabled the WAL entirely since we don't need recovery after pod restarts or rollouts. Increasing memtable sizes and raising the `MemTableStopWritesThreshold` before write stalls improved write throughput as well.
...
Shadow mode was worth the extra setup. It let us validate the proxy's behavior against real production traffic without affecting live requests. Once that looked solid, we enabled the proxy behind per-account feature flags.
