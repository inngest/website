...
Check out the [v2 migration guide](/docs/reference/typescript/v3/migrations/v2-to-v3?ref=blog-sdk-2-0) to learn how to upgrade today.
...
Check out [Defining Event Payload Types](/docs/reference/typescript/v4/client/create?ref=blog-sdk-2-0#defining-event-payload-types) to learn about specifying types in v2.

### Update: Standard Schema support (September 2025)

Since v2, Inngest's TypeScript SDK has evolved to support the Standard Schema interface, expanding beyond Zod to support multiple validation libraries:

```typescript
import { z } from "zod";  // Now supports Zod 4!
import * as v from "valibot";

// Choose your preferred validator
const schemas = new EventSchemas()
  .fromZod(zodSchemas)      // Zod 4 supported
  .fromValibot(valibotSchemas)  // New option
  .fromArkType(arkSchemas);     // New option
```

This gives you flexibility to use Zod 4, Valibot (smaller bundles), ArkType (TypeScript-first), joi, or any [Standard Schema](https://standardschema.dev/)-compatible library.

Available in SDK v3.42.0+. [Learn more](/docs/reference/typescript/v4/client/create#defining-event-payload-types)
...
- Error monitoring (check out our [Sentry example](/docs/reference/typescript/v4/middleware/examples?ref=blog-sdk-2-0#sentry-error-reporting-and-tracing))
- Data transformations
- Logging support (check out our [logging example](/docs/reference/typescript/v4/middleware/examples?ref=blog-sdk-2-0#logging))
...
We're excited to see what you'll create with v2! Check out the [v2 migration guide](/docs/reference/typescript/v3/migrations/v2-to-v3?ref=blog-sdk-2-0) to learn how to upgrade today.
