...
We encourage all users to upgrade.  Function versioning and lifecycle management is a large improvement. Check out the [v3 migration guide](/docs/reference/typescript/v3/migrations/v2-to-v3?ref=blog-sdk-3-0) to learn how to upgrade today.
...
### Standard Schema support (September 2025, SDK v3.42.0+)

Since the v3.0 release, Inngest has added support for the Standard Schema interface, allowing you to use any validation library:

- **Zod 4** - Now fully supported
- **Valibot** - Smaller bundle sizes  
- **ArkType** - TypeScript-first validation
- **joi** - Battle-tested validation
...
inngest.createFunction(
  ...
  { 
    event: "user.created",
    schemas: { data: yourPreferredSchema }  // Works with any Standard Schema validator
  },
  async ({ event }) => {
...
[Learn more](/docs/reference/typescript/v4/client/create#defining-event-payload-types) | [View changelog](https://www.inngest.com/changelog/2025-09-26-zod-4-standard-schema-support)

With v3 as the foundation going forwards, it's much easier to iterate on and develop new features.  We're excited to see what you'll create with v3! Check out the [v3 migration guide](/docs/reference/typescript/v3/migrations/v2-to-v3?ref=blog-sdk-3-0) to learn how to upgrade today.
