...
We restarted the poolers at 16:26. This was after the pools had already saturated, and it did not address the cause: the blocking transactions were still running, so the pools refilled. The restart also surfaced an unrelated problem, a missing runtime directory, which slowed our recovery further.
...
Recovery came at approximately 18:12. One detail is worth calling out: Postgres lock counts had already fallen back to baseline by 18:00, while the poolers stayed saturated for another eight to ten minutes. Once every client slot is held by a waiting query, the pooler does not recover on the database's timescale — the queue has to drain first. By 18:15 waiting clients were at zero and query times were back to 15–16 ms.
...
- **16:09:** Pooler client queues begin growing.
- **16:16:** Database lock counts peak at roughly seventeen times baseline.
- **16:18–16:19:** Our poolers fill and begin rejecting connections and disconnecting clients at the 120 second wait timeout.
- **16:21:44:** We declare an incident and page on-call.
- **16:22:47:** `max_client_conn` errors are confirmed as the immediate failure.
- **16:26:51:** We restart the poolers. The pools refill, because the blocking transactions are still running.
...
- **18:51:** A pooler configuration change merges, after recovery.
...
3. **Blocked queries consume pool capacity.** A query waiting on a lock holds its client slot in the pooler for the full wait. Lock contention on a handful of tables was therefore converted into total connection exhaustion for every service sharing the pool.
...
- **Adding headroom and isolation at the pooler.** We have adjusted pooler configuration and are reviewing pool sizing so that a single saturating workload cannot consume the connection capacity that unrelated services depend on. We are also looking at how to drain a saturated pool deliberately rather than waiting for it to clear.
- **Improving visibility into database contention.** We are expanding our pooler and Postgres dashboards and alerting so that rising lock counts and client wait times page us before the pool is exhausted, rather than after dependent services begin failing.
- **Fixing recovery friction.** The missing runtime directory encountered during the pooler restart, and the SSH access gaps that slowed responders from reaching the pooler hosts, are both being addressed so that operating this tier during an incident is faster.
