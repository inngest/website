...
## Stream your agent to the frontend

Once you've built your agent workflow with AgentKit, you can stream real-time updates to your users with the `useAgent` React hook:
```tsx
...
No WebSocket management, no state stores, no connection handling—just durable, observable AI workflows streaming to your UI.

[Read the useAgent announcement](/blog/agentkit-useagent-realtime-hook).
...
- **Add your own features.** We invite you to extend the app. Here are some ideas that could make this app even more useful:
  ...
  3.  Stream agent responses to a web UI using the `useAgent` hook instead of (or in addition to) email.
