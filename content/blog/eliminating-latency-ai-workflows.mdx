...
```tsx
import { Inngest } from "inngest";
import { serve } from "inngest/connect";
...
serve({
  client: inngest,
  functions: [processAgentLoop, handleWebhook],
});
```
...
const name = await step.run("get-name", () => user.name);
const email = await step.run("get-email", () => user.email);
...
**Inter-step latency** is the time between one step completing and the next beginning. This is pure orchestration overhead. Track it with middleware:

```tsx
const latencyMiddleware = new InngestMiddleware({
  name: "latency-tracker",
  init: () => ({
    onFunctionRun: ({ fn }) => {
      let lastStepEnd: number | null = null;
      return {
        transformOutput: ({ step }) => {
          const now = Date.now();
          if (lastStepEnd) {
            metrics.histogram("inngest.inter_step_latency_ms", now - lastStepEnd, {
              function: fn.id, step: step?.id,
            });
          }
...
- [Performance documentation](https://www.inngest.com/docs/improve-performance) for setup guides on checkpointing and Connect
- [The Principles of Durable Execution](https://www.inngest.com/blog/principles-of-durable-execution) for how durable execution works under the hood
- [How we built a fair multi-tenant queue](https://www.inngest.com/blog/building-the-inngest-queue-pt-i-fairness-multi-tenancy) for the queue architecture deep dive
