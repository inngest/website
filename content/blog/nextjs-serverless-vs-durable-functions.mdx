...
showSubtitle: true
subtitle: Learn how Durable Functions remove the need of a separate server to handle long-running workflows or to power queues.
date: 2024-08-13
...
![Diagram illustrating Resend and Mintlify Next.js applications architecture overview. Both applications rely on some backend parts responsible for either scheduling emails or sync with third party services like GitHub or Algolia.](/assets/blog/nextjs-serverless-vs-durable-functions/1.png)
...
![This illustration compares two diagrams. The first shows that a Serverless Function is meant to deal with direct user interactions while the second diagram showcases a Serverless Function calling a Durable Function to handle an onboarding workflow performing multiple steps and sending an email to the end user.](/assets/blog/nextjs-serverless-vs-durable-functions/2.png)
...
![This diagram illustrates how the domain verification workflow get streamlined when developed with a Durable Function. The account creation endpoint triggers the workflow which run its steps for each domain.](/assets/blog/nextjs-serverless-vs-durable-functions/4.png)
...
<CTACallout text={'Want to get started with Durable Functions in Next.js?'} cta={{ href: '/docs/getting-started/nextjs-quick-start?ref=blog-nextjs-serverless-vs-durable-functions', text: 'Read our tutorial' }} />


## Durable Functions: under the hood

Durable Function are composed of 2 main components: [Triggers](/docs/features/events-triggers) and [Steps](/docs/features/inngest-functions/steps-workflows).
...
![Durable Functions are run using four main triggers: an event sent from a Serverless Function or another Durable Function, a webhook or a CRON event.](/assets/blog/nextjs-serverless-vs-durable-functions/5.png)
...
Events are sent to a[ Durable Execution Engine (for example, Inngest)](/docs/learn/how-functions-are-executed), which triggers Durable Function runs and manages their steps (like React manages Components and their state).
...
![This diagram illustrates how an event sent from a Serverless Function triggers a Durable Function run. The event is first sent to the Durable Execution Engine which requests the Next.js application at a dedicated API Routes where all Durable Funtions are registred.](/assets/blog/nextjs-serverless-vs-durable-functions/6.png)
...
![This diagram illustrates how the Durable Execution Engine interact with the Next.js applications. Each step of a Durable Function results in a communication between the Durable Execution Engine and the Next.js application where the Durable Execution Engine sends state and instructions (ex: "run step 1") and the response contains the state of the executed step.](/assets/blog/nextjs-serverless-vs-durable-functions/7.png)
...
## Looking for a durable execution engine?

If you're looking for a durable execution engine that extends Durable Functions beyond traditional background workflows—enabling you to add durability directly to API routes and achieve near-zero latency for real-time applications—Inngest offers these capabilities:

### Beyond workflows: step.run() in REST APIs (September 2025)

Durable Functions aren't just for background workflows anymore. With Inngest's latest releases, you can now add durability directly to your API routes:
```javascript
...
Crash the server halfway through—Inngest resumes from the last completed step. No queue, no separate worker, just one code path with automatic durability and observability.

[Learn more about step.run() in REST APIs](/docs/learn/rest-endpoints) | [View changelog](https://www.inngest.com/changelog/2025-09-22-step-run-in-apis-golang)

### Near-zero latency with Checkpointing (December 2025)

For workflows requiring real-time responsiveness, **Checkpointing** eliminates the latency tradeoff traditionally associated with durable execution:

- Inter-step latency drops to milliseconds
- 50% reduction in overall workflow duration
- Full durability maintained through background checkpoints

This makes Durable Functions viable for interactive, user-facing experiences in Next.js applications—not just background jobs.

[Learn more about Checkpointing](/docs/setup/checkpointing) | [View changelog](https://www.inngest.com/changelog/2025-12-10-checkpointing)
