---
heading: How to add background jobs with real-time updates to an Next.js application
...
Backgound jobs are core to web-applications, keeping the user experience responsive by moving some backend work outside of the API.


With background jobs, any backend work not directly tied to the user experience or slow operations such as importing contacts is moved into asynchronous back-end operations.
These background jobs can send updates upon completion, for example, via emails, or send realtime updates with streamed messages to keep users informed of the process (for example, sending a batch of emails)


In this article, you'll learn how to add background jobs with real-time updates in Next.js using Inngest.

## Project overview: CampaignCraft

CampaignCraft is a Next.js application that helps in sending personalized email campaign by automatically
segmenting the uploaded contacts CSV files and AI campaigns drafts.

_TODO: video_

CampaignCraft is built with the following tech:
- **Next.js** - The full-stack application framework
- **Shadcn** - The UIKit library providing all UI components
- **Vercel** - The defacto deployment platform for Next.js applications
...
The two primary features of CampaignCraft, automatically segmenting uploaded contacts file and sending campaigns
are leveraging background jobs with realtime updates for a smoother user experience.

The final project is [available on GitHub](#TODO) along with README setup instructions.

## Implementation overview

Our project relies on two main concepts to deliver a smooth user experience: background jobs and realtime updates.
...
When invoked, a background job gets added to a queue processed by a background jobs engine (ex, Inngest) that pick background jobs from the queue in the right order.
On top of moving time-consuming work outside of the API, background jobs also enables to retry failed jobs, bringing more reliability to backend work coupled to external services like LLM APIs or email providers:
...
While this guide focuses on two primary use cases requiring background jobs, improving the user experience or scaling your web applications
is a wider topic involving other concepts such as scalability with Flow Control, recovery tactics for production use cases and more.

Inngest is an orchestration engine enabling developers to build, operate and scale background jobs and workflows.
Its TypeScript provide features ranging from simple background jobs to more advanced scalable workflows designed for scale.
On top of its SDKs, Inngest also provides a platform providing observability (charts & usage), recovery tools (bulk cancelations, replay of jobs) and
integrations with other platforms like Vercel. Finally, Inngest's Realtime feature provide an API to send realtime updates to users for long running background jobs.

You can install and configure Inngest to your Next.js app in under 5-minutes [by following this quickstart](/docs/getting-started/nextjs-quick-start?ref=blogpost-nextjs-bg-jobs-realtime).
...
Let's start by installing and configuring Inngest to CampaignCraft.
...
Then, we will create a new Next.js API Routes to enable the Inngest orchestration engine to trigger background jobs defined in the application:
...
Inngest is now setup in the Next.js app. Let's now look at how to implement the contacts importer feature using a background job.

## Import contacts from a background job

CampaignCraft's first main feature is a CSV contacts import which segment the newly imported contacts using AI (OpenAI).
...
- A single LLM request failure will cancel the whole import, without retry
...
- Enforcing automatic isolated retries, making the import and AI processing more resilient
...
<aside className="max-w-[65ch] m-auto bg-indigo-900/20 text-indigo-100 leading-relaxed rounded-lg py-0 px-6  my-12 border border-indigo-900/50">
  **Inngest background jobs are triggered using events**
  ...
  You can see events as arguments of backgound jobs. `inngest.send()` is used to send an event to Inngest which will enqueue an execution of our `contact-import` background job.
</aside>
...
<aside className="max-w-[65ch] m-auto bg-indigo-900/20 text-indigo-100 leading-relaxed rounded-lg py-0 px-6  my-12 border border-indigo-900/50">
  **Inngest background jobs are composed of steps**
  ...
  The `step.run()` block present in the `contact-import` background jobs helps in dividing our backgroud job into
  individual retriable units. When a steps fails, this enables Inngest to recover your background jobs from where it failed,
  without re-running all the steps.
  This prevents any unwanted duplicated database inserts or costly LLM requests.
</aside>

Let's now create a background job sending realtime updates for the campaign sending feature.


## Sending campaigns using a realtime background job

Sending a campaign is a quicker process than importing contact.
Still, it benefits from running inside a background job to enable the user to leave the page before it completes or
to make it more resilient to failures.

To keep the user informed of the progress of the batch sending of emails, our `campaign-send` background jobs will send regular
realtime updates.
...
Then, below the `contact-import` inngest function, let's define a unique channel that we will later use to send realtime messages:
...
<aside className="max-w-[65ch] m-auto bg-indigo-900/20 text-indigo-100 leading-relaxed rounded-lg py-0 px-6  my-12 border border-indigo-900/50">
  **Inngest Realtime: channels and topics**
  ...
  Inngest Realtime enables you to send realtime updates from background jobs to React components using 2 concepts: channels and topics.

  - Channels contains different types of message belonging to the same background job, for example: a batch sending of emails for a given campaign.
...
Let's now update our front-end (React components) to subscribe to the realtime updates!

### Subscribing to Realtime messages from the front-end

Subscribing from realtime updates sent from Inngest background requires two changes.

First, create a `app/actions.ts` file as follows:
...
<aside className="max-w-[65ch] m-auto bg-indigo-900/20 text-indigo-100 leading-relaxed rounded-lg py-0 px-6  my-12 border border-indigo-900/50">
  **Inngest Realtime: subscription token**
  ...
  Inngest Realtime requires to create a Server Action to securely fetch a subscription token from the server.

  This subscription token is later used by the `useInngestSubscription()` React hook to subscribe to live updates.
</aside>

Let's now add the `useInngestSubscription()` to our Next.js page to subscribe to live updates:
...
CampaignCraft is now leveraiging two background jobs powering a smooth user experience, let's test it out!
...
Now navigate back to the home page, and click on the "Create Campaign" button. Create a campaign by selecting a segment, generate some AI content and send it. You will see the campaign being sent with realtime updates.

_TODO: video 2_

## Conclusion

In this article, we explored how to implement realtime updates for background jobs in a Next.js application using Inngest Realtime. The main concepts covered include:
...
- **Inngest Realtime**: Leveraging Inngest's realtime capabilities to send live updates from background jobs to the frontend, enabling users to see progress and results as they happen.
- **Channels and Topics**: Organizing realtime messages using channels (grouping messages for a specific backgound job) and topics (categorizing message types within a channel).
...
By combining these concepts, you can build responsive, user-friendly applications that keep users informed about the status of their background processes in realtime.
...
You can limit the number of concurrent runs for a given background job by configuring a `concurrency`.
...
Background are retried up to 4 by default and can be configured using the `retries` option.
The example below showcases a function with no retries:
...
The `useInngestSubscription()` React hook provided by `@inngest/realtime` will resume any lost connection automatically.
Any permanent lost of connection will be reflected in the `error` and `state` properties returned by the hook:
...
Every subscription is bound to a channel and its one or multiple topics.

To subscribe to multiple topics from a given channel, list them in the `topics` arguments passed to `getSubscriptionToken()`:
...
To subscribe to multiple channels, create multiple subscription tokens and perform one call to `useInngestSubscription()` per channel.

**Q: How do I handle errors or retries in the background job and reflect them in the UI?**

Inngest offers multiple ways to handle errors, described [in this guide](/docs/guides/error-handling).