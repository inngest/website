...
To make it multi-player, we're going to explore [swappable sandboxes, external state and memory](/blog/owning-your-ai-agents-starts-with-the-execution-layer?ref=blog-your-agent-needs-a-harness-not-a-framework). This would enable Utah to run on serverless if someone wanted to.

There are a lot of fun UX things we'd like to add built on the Inngest API and our Insights feature to build session monitoring for coding sessions. We will also explore using `step.waitForEvent()` to create [human-in-the-loop approval flows](/docs/ai-patterns/human-in-the-loop?ref=blog-your-agent-needs-a-harness-not-a-framework) when more input is needed.

The last piece that we're exploring to make this truly “universally triggered” is [enabling Utah to write itself](/blog/agent-loop-architecture?ref=blog-your-agent-needs-a-harness-not-a-framework) — building new agents and workflows, creating new webhooks, and monitoring itself via API. If you're interested, share some ideas on the GitHub repo.
