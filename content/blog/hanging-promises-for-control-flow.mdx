...
You can't cancel a JavaScript promise. There's no `.cancel()` method, no `AbortController` integration, no built-in way to say "never mind, stop." The TC39 committee [considered adding cancellation](https://github.com/tc39/proposal-cancelable-promises) in 2016, but the proposal was withdrawn after heated debate.
...
```ts
function* myWorkflow(step) {
  let data;
  try {
    data = yield step.run("fetch-data", async () => fetchData());
  } catch {
    console.log("Failed to fetch data, using default");
    data = defaultData;
  }

  yield step.run("process-data", async () => processData(data));
}
```

Internally, you could have a driver that's calling `.next()`. To interrupt, it just stops calling `.next()`.

```ts
// The runtime drives the generator
const gen = myWorkflow(step);

// Runs until the first yield 
const firstStep = gen.next();

// To interrupt, just don't call gen.next() again. The catch block never runs.
// The generator is frozen mid-yield.
```
...
Generators also break down with concurrency. With `async`/`await`, running steps in parallel is natural:

```ts
const results = await Promise.all([
  step.run("a", async () => fetchA()),
  step.run("b", async () => fetchB()),
  step.run("c", async () => fetchC()),
]);
```
