...
Now let’s dig in. Every agent is a loop. Think → act → observe → repeat. The LLM decides what to do, you execute the tool, feed the result back, and let the LLM decide again. It keeps going until the model responds with text instead of a tool call. (For a concise reference, see the [agent tool loop pattern guide](/docs/ai-patterns/agent-tool-loops?ref=blog-ai-agents-inngest-durable-steps).) Simplifying, often in agents you’ll do the following steps:
...
A single agent loop gets you surprisingly far. But some tasks are better run in isolation: long-running research, background file processing, scheduled reminders. That's where [sub-agents](/docs/ai-patterns/sub-agent-delegation?ref=blog-ai-agents-inngest-durable-steps) come in.
