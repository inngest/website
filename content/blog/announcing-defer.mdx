...
**Steps work inside deferred runs.** The full power of Inngest — `step.run()`, `step.waitForEvent()`, `step.sleep()` — is available inside the deferred handler. This is what makes patterns like [outcome-based scoring](/blog/introducing-scoring?ref=blog-announcing-defer) possible.
...
`defer()` is available today as an experimental API in the Inngest TypeScript SDK. Check the [deferred functions documentation](/docs/reference/typescript/v4/functions/deferred-functions) for the full API reference. For attaching scores to runs—including deferred, outcome-based scoring—see [Introducing Scoring](/blog/introducing-scoring?ref=blog-announcing-defer).
