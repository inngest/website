...
Inngest's SDKS allows you to create waits through several methods, one of which is the [`step.waitForEvent`](/docs/reference/typescript/v4/functions/step-wait-for-event?ref=blog-what-are-durable-functions) function.
...
## Making HTTP requests durable with step.fetch()

Beyond `step.run()` and `step.waitForEvent()`, Inngest now provides `step.fetch()` for making durable HTTP requests to third-party APIs:
```javascript
const processFiles = inngest.createFunction(
  { id: "process-files" },
  { event: "files/process" },
  async ({ step }) => {
    // The request is offloaded to Inngest
    const response = await step.fetch("fetch-files", 
    ...
    // Your function resumes here with the response
...
Benefits of `step.fetch()`:
- Requests execute on Inngest's infrastructure, not your serverless runtime
- Automatic retries on network failures
- No impact on your function's execution time limits
- Full observability of all HTTP requests in the dashboard

[Learn more about step.fetch()](/docs/reference/typescript/functions/fetch)
