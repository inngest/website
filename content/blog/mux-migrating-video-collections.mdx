...
Once we have the list of videos, we map over them, building up an array of `truckload/video.process` events, and fire them off in a batch using [`step.sendEvent()`](/docs/reference/typescript/v4/functions/step-send-event).
...
Inngest's [encryption middleware](/docs/reference/typescript/v4/middleware/examples#e2-e-encryption) meets this requirement perfectly. The user can supply their source provider and destination credentials on the client, and as soon as these sensitive details hit the server, they're encrypted.
