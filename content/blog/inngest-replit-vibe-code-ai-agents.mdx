...
Now, anyone can [vibe code](https://blog.replit.com/what-is-vibe-coding) durable agents that scale. Replit brought Inngest into [Agent 3](https://replit.com/agent3) so they could give their users all of the durability, retries, state, and observability they would have had to cobble together on their own.
...
With Agent 3, [durable orchestration](/blog/principles-of-durable-execution?ref=blog-inngest-replit-vibe-code-ai-agents) comes baked-in to the agents it creates—zero infrastructure knowledge required. Here’s how it works:
...
2. Replit launches the [Inngest dev server](/docs/local-development?ref=blog-inngest-replit-vibe-code-ai-agents) in the dev version of your Replit app.
...
- **Built-in Observability**: Every execution is logged with [full traces](/docs/platform/monitor/inspecting-function-runs?ref=blog-inngest-replit-vibe-code-ai-agents) which you can examine yourself in Inngest. No more guessing what went wrong.
...
- **Rate Limit Handling**: Inngest's [concurrency controls](/docs/guides/concurrency?ref=blog-inngest-replit-vibe-code-ai-agents) prevent the agent from overwhelming external APIs. Process 5 messages/minute instead of trying 100 at once.
...
It turns out, the `step.run` API makes building similar things yourself simple.  Before using Inngest, you’d have to provision workers, queues, and implement a bunch of state yourself.
