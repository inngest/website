...
## Even faster AI workflows with Checkpointing

For AI applications requiring real-time responsiveness—like interactive chat interfaces or live agent workflows—Inngest now offers **Checkpointing** to eliminate inter-step latency while maintaining durability.

When building AI workflows with `step.ai.infer()` and `step.run()`, you can now enable checkpointing to achieve:
...
Checkpointing is available in developer preview. [Learn more](/docs/setup/checkpointing).
