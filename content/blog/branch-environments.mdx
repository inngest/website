...
Since this article was published, we've enhanced the local development experience to complement Branch Environments. The Inngest dev server now supports MCP (Model Context Protocol), allowing you to connect Claude Code, Cursor, and other AI assistants directly to your local Inngest functions. This makes it even easier to test and iterate on your workflows locally before deploying to branch environments:
...
This pairs perfectly with Branch Environments—test locally with AI assistance, then deploy to isolated branch environments for full-stack testing.
