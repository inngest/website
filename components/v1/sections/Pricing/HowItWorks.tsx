...
// "HOW IT WORKS" — same Embla carousel mechanics as Home/HowItWorks
// (drag, loop, auto-progress, AdvanceClick) with the pricing-page
// heading row + "See Docs" CTA layered above the carousel. Icons
// reuse the shared /assets/v1/how-it-works/ glyphs so the visuals
// stay in lockstep with the home page.
...
// Icons are the same three feature-cards SVGs the Home FeatureCards
// section uses (retries / flow-control / observability). Widths and
// heights mirror their natural viewBox aspect — bumped to ~260 px
// so they read as carousel illustrations rather than card glyphs.
const STEPS: Step[] = [
  {
    number: "01",
  ...
  {
    number: "02",
    title: "Connect Inngest with one command",
    body:
      "Run npx inngest-cli dev locally. Inngest discovers your functions instantly and gives you a real-time dev server with traces, logs, and a UI — zero config.",
    iconSrc: "/assets/v1/feature-cards/flow-control.svg",
    iconWidth: 300,
    iconHeight: 210,
    iconColWidth: 338,
  },
  {
    number: "03",
    title: "Deploy & forget the infra",
    body:
      "Push your code. Inngest handles the orchestration — retries, concurrency, throttling, scheduling, and observability — all without workers or queues on your end.",
    iconSrc: "/assets/v1/feature-cards/observability.svg",
    iconWidth: 260,
    iconHeight: 214,
    iconColWidth: 260,
  },
...
const DOCS_URL = "/docs?ref=pricing";
...
function Carousel() {
  ...
  return (
    <div
      ref={rootRef}
      className="flex flex-col gap-[41px] lg:gap-v1-stack-lg"
    >
      <AdvanceClick onAdvance={next} ariaLabel="Show next step" hideCursor>
        ...
        >
          {/* Padding-right on each slide rather than CSS `gap` so the
              loop-wrap seam keeps its real spacing (see Home/HowItWorks
    ...
    >
      ...
      <div className="flex w-1/2 shrink-0 flex-col gap-[19px] lg:w-[368px] lg:gap-[14px]">
        ...
        <div className="flex flex-col gap-3 lg:gap-[14px]">
          <h3 className="text-[24px] leading-[1.2] text-v1-frost lg:text-balance lg:text-v1-heading-card">
            {step.title}
          </h3>
          <p className="text-pretty text-[16px] leading-[1.2] tracking-[-0.01em] text-v1-frost lg:text-v1-body-lg-loose lg:tracking-normal">
            {step.body}
          </p>
        </div>
      </div>
...
function ProgressRail({
  ...
  return (
    <div className="flex items-center gap-[13px] px-[13px]">
      ...
      >
        <CarouselArrow
          className={`size-[31px] rotate-180 sm:size-10 ${arrowSvg}`}
        />
      </button>
