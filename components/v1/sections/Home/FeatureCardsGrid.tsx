...
export interface FeatureCard {
  label: string;
  /** Icon vector. Optional — omit for icon-less cards (e.g. the
   *  compare-to-temporal "Why choose" row). */
  vector?: string;
  vectorWidth?: number;
  vectorHeight?: number;
  /** Extra classes on the icon container (e.g. `items-center`). Default
   *  alignment is `items-end`. */
  iconClassName?: string;
  /** Override the icon container style — merged after the default
   *  `height: clamp(54px,6vw,72px)` so callers can grow the box (the
   *  compare-to-temporal cards use a taller 90px box). */
  iconStyle?: React.CSSProperties;
  /** Override the rendered icon `<img>` style — merged after the default
   *  `width: clamp(60px,7vw,vectorWidth)` for larger icons. */
  vectorStyle?: React.CSSProperties;
  /** Optional tagline rendered directly under the title. */
  subtitle?: string;
  /** Per-card class overrides (merged via tailwind-merge, so they win
   *  over the defaults) — used by CompareTemporal/WhyChoose to size the
   *  title/subtitle/body without affecting the other cards. */
  titleClassName?: string;
  subtitleClassName?: string;
  bodyClassName?: string;
  /** Override the article's layout (padding / justify / etc.). Merged
   *  last so it wins — CompareTemporal/WhyChoose uses it to hit the
   *  `px-20 py-64`, `justify-between` card geometry. */
  cardClassName?: string;
  /** Override the title→subtitle block gap (default `gap-2`). */
  titleBlockClassName?: string;
  /** Render the body as a single naturally-wrapping paragraph instead
   *  of the designer-locked per-line `whitespace-nowrap` spans. */
  bodyWrap?: boolean;
...
// TEST VARIANT — both hover-in AND hover-out use `springs.glide`
// (over-damped, zero overshoot). The live FeatureCards uses
// `springs.lift` on hover-in which is intentionally a touch
// underdamped (stiffness 110, damping 12) and overshoots slightly
// for a "lift with bounce" feel — design review wanted that bounce
// removed: cards should expand to exact point and retract to exact
// point. Same `glide` spring on both directions gives a clean
// asymmetry-free settle.
const HOVER_IN = springs.glide;
...
/**
 * The card row + serialised hover hand-off. Reused under a section's
 * own heading/padding (e.g. CompareTemporal/WhyChoose). Supply your
 * own `cards`.
 */
export function FeatureCardsGrid({
  ...
  cards: FeatureCard[];
  /** When set, the card at this index is "active" by default and the
   *  grid falls back to it whenever nothing is hovered (instead of going
   *  fully idle). Used by CompareTemporal/WhyChoose so the left card
   *  loads pre-flooded and hover merely transfers the active styling.
   *  Omit for the home grid's hover-only behaviour. */
  idleActiveIndex?: number;
}) {
  // The label the grid rests on when idle (null = fully idle, the
  // home-page default).
  const idleId =
    idleActiveIndex != null ? (cards[idleActiveIndex]?.label ?? null) : null;
    ...
    <div className="has-hover-dim grid w-full grid-cols-1 sm:grid-cols-3 sm:items-stretch sm:[width:round(down,100%,3px)]">
      {cards.map((card, idx) => (
        <Card
          ...
          onLeave={() =>
            setPendingId((prev) => (prev === card.label ? idleId : prev))
          }
        />
      ))}
    </div>
...
function Card({
  ...
  useEffect(() => {
    const controls = animate(
      ...
      isActive ? HOVER_IN : RELEASE_BOUNCE,
    );
  ...
  return (
    <motion.article
      data-feature-card
      // Mirrors the hover state for the idle-active card (e.g. the left
      // "Why choose" card): overlays keyed to `group-data-[active=true]`
      // flood salmon + noise without a cursor. Spotlight stays hover-only.
      ...
      // every size.
      className={cn(
        "group relative isolate flex flex-col items-start justify-center gap-4 p-4 [--lift-y:0px] [--surface-y:0px] sm:gap-[25px] sm:p-5 lg:[--lift-y:var(--lift)] lg:[--surface-y:var(--surface-extra-y)]",
        card.cardClassName,
    ...
    >
      ...
      <span
        aria-hidden="true"
        style={followsExtraY}
        className={`pointer-events-none absolute inset-0 -z-10 rounded bg-v1-accent-salmon-gradient opacity-0 motion-safe:transition-[opacity,border-radius] motion-safe:duration-[600ms] motion-safe:ease-v1-lift group-hover:!rounded group-hover:opacity-100 group-focus-within:!rounded group-focus-within:opacity-100 group-data-[active=true]:!rounded group-data-[active=true]:opacity-100 ${overlayEdgeFlat}`}
      />
      ...
      <span
        ...
        className={`pointer-events-none absolute inset-0 -z-10 bg-cover bg-center opacity-0 motion-safe:transition-[opacity,border-radius] ${dur} ${ease} group-hover:!rounded group-hover:opacity-100 group-focus-within:!rounded group-focus-within:opacity-100 group-data-[active=true]:!rounded group-data-[active=true]:opacity-100 ${overlayEdgeFlat}`}
      />
      ...
      <div
        className={cn(
          "flex w-full flex-col items-start gap-[21px]",
          // The 8px breathing room is icon-alignment slack; icon-less
          // cards (WhyChoose) drop it so the title sits flush to the
          // card's own padding.
          card.vector && "py-2",
        )}
      >
        {/* Icon container height + icon render width clamp against
            ...
            and the clamp pins to its max. Omitted when the card has no
            vector (icon-less cards). */}
        {card.vector && (
          <div
            className={cn("flex w-full items-end", card.iconClassName)}
            style={{ height: "clamp(54px, 6vw, 72px)", ...card.iconStyle }}
          >
        ...
        {/* Title + optional subtitle tagline (tagline sits tight under
            the title; the article's flex gap provides the larger gap
            down to the body). */}
        <div className={cn("flex w-full flex-col gap-2", card.titleBlockClassName)}>
          <h3
            className={cn(
              "whitespace-nowrap text-[20px] leading-[1.25] tracking-[-0.01em] text-v1-frost lg:text-[clamp(15px,1.6vw,22px)]",
              card.titleClassName,
            )}
          >
            {card.label}
          ...
          {card.subtitle && (
            <p
              className={cn(
                "text-v1-body-sm leading-[1.4] text-v1-frost/80",
      ...
      <p
        className={cn(
          "w-full text-pretty text-v1-frost text-[15px] leading-[1.5] lg:text-[clamp(0.8125rem,1.25vw,1.125rem)]",
          card.bodyClassName,
        )}
      >
        {card.bodyWrap
