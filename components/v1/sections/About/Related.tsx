...
function RelatedCard({
  ...
  return (
    ...
    >
      ...
      {/* Title — Heading/Md (32 px / 40 line / -0.01em). The fixed
          `h-[72px]` + `line-clamp-2` only matter for row alignment in the
          2-col (sm+) grid; on mobile (single column) a long title needs 3
          lines, so drop the height cap AND the clamp there and let it show
          in full. */}
      <h3 className="min-h-[72px] text-v1-heading-md-cap text-white sm:line-clamp-2 sm:h-[72px] sm:min-h-0">
        {title}
      </h3>
