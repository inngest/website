...
import { useLanguageStore, SDK_LANGUAGES } from "./LanguageStore";
import {
  GUIDE_KEY_TO_SDK,
  SDK_TO_GUIDE_KEY,
...
export function CodeGroup({
  ...
  const selectedIndex = useMemo(
    ...
    [globalLanguage, currentLanguage, languages, preferredLanguages]
  );
  ...
  const handleChange = (newSelectedIndex: number) => {
    ...
    const { preferred, sdkLanguage } = resolveLanguageSelection(
      languages[newSelectedIndex]
    );
    preventLayoutShift(() => {
      addPreferredLanguage(preferred);
      if (sdkLanguage) {
