  ...
  return (
    <div
      className={clsx(
        "my-6",
        nested &&
          "-mt-3 rounded-md border border-slate-900/5 pt-2 dark:border-white/5",
    ...
    >
      {nested && (
        <div
          className={clsx(
            "border-b border-slate-900/5 px-3 pb-2 text-xs font-semibold",
  ...
  return (
    <li id={name} className="m-0 scroll-mt-24 px-0 py-4 first:pt-0 last:pb-0">
      <dl className="m-0 flex flex-wrap items-center gap-x-3 gap-y-2">
        ...
        <dd className="font-mono text-xs font-medium text-slate-500 dark:text-slate-400">
          {type}
        </dd>
        {!attribute && (
          <>
...
export function VersionBadge({ version }: { version: `v${string}` }) {
  return (
    <span className="inline-flex items-center rounded-full bg-slate-200 px-3 py-0.5 text-xs font-medium leading-4 text-slate-800 dark:bg-slate-800 dark:text-slate-200">
      <span>{version}</span>
    </span>
  );
}
...
export function Card<T extends React.ElementType = "div">({
  ...
  return (
    <Component
      className={clsx(
        "not-prose group relative my-2 w-full overflow-hidden rounded border border-carbon-200 font-normal ring-2 ring-transparent dark:border-carbon-700",
        !!props.href &&
          "cursor-pointer hover:border-carbon-300 hover:bg-carbon-50/50 dark:hover:border-carbon-600 dark:hover:bg-carbon-900",
        className
      )}
      {...props}
    >
      <div
        className={clsx("flex items-start gap-1 px-6 py-6", {
          "flex-row": iconPlacement === "left",
          "flex-col": iconPlacement === "top",
        })}
      >
        {icon && iconPlacement === "top" && (
          <div className="my-1 flex min-w-7 justify-start">{icon}</div>
        )}
        <div>
          <div className="mb-1 flex items-center">
            {icon && iconPlacement === "left" && (
              <div className="mr-1 flex min-w-7 justify-start">{icon}</div>
            )}
            <h2
              className={clsx(
                "mb-px text-sm font-semibold text-[#2E2E2E] dark:text-carbon-50",
                {
          ...
          <span
            className={
              "mt-1 text-sm font-medium text-carbon-700 dark:text-carbon-200"
            }
          >
            {children}
          </span>
...
export function Steps({ children }: { children: React.ReactNode[] }) {
  return (
    <div className="mb-6 mt-10 pl-3.5" role="list">
...
export function Step({
  ...
  return (
    <div className="step w-full overflow-hidden pl-12 pr-px">
