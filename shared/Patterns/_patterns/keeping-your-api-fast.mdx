...
Functions are defined by declaring which event(s) should trigger them. When matching events are received, all corresponding functions run in the background automatically. Any returned value is logged and any error thrown will inform Inngest to [retry the function](/docs/features/inngest-functions/error-retries/inngest-errors?ref=patterns).
...
Functions can be [deployed in several ways](/docs/apps/cloud?ref=patterns) that fit your current stack or needs.
...
* [Docs: Writing Functions](/docs/learn/inngest-functions?ref=patterns)
