...
subtitle: Fan out to hundreds or thousands of independently-retried jobs from a single trigger, from batch AI processing to multi-tenant report generation.
pattern: events
tags: [Event Coordination, Architecture, AI]
---

Sometimes you need to reliably trigger hundreds or thousands of jobs at once, making sure that every job succeeds independently. For example, you might run nightly batch inference across 10,000 documents, fan out embedding generation for a RAG pipeline, or trigger product imports for thousands of tenants.
...
Inngest supports scheduled functions and event-triggered functions. Combining the two enables you to [fan-out functions to run in parallel](/docs/patterns/running-functions-in-parallel?ref=patterns). We'll define two these two functions:
...
This is the system &mdash; both functions can even be defined in the same file to keep things simple and maintainable. This approach works well for systems that have commonly scheduled times, but for more flexible systems that are scheduled as one-off, non-repeated tasks you should review [Patterns: Running at specific times](/docs/patterns/running-at-specific-times?ref=patterns).
...
* [Patterns: Running at specific times](/docs/patterns/running-at-specific-times?ref=patterns)
