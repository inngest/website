...
subtitle: Fan out to multiple model calls, evaluations, or processing tasks from a single event.
pattern: events
...
const classify = inngest.createFunction(
  { id: "classify-document", triggers: [{ event: "document/uploaded" }] },
  async ({ event, step }) => {
    const content = await step.run("fetch-content", async () => {
      return await storage.getContent(event.data.documentId);
    });
...
const extractEntities = inngest.createFunction(
  { id: "extract-entities", triggers: [{ event: "document/uploaded" }] },
  async ({ event, step }) => {
    const content = await step.run("fetch-content", async () => {
      return await storage.getContent(event.data.documentId);
    });
...
* [Patterns: Reliably run critical workflows](/docs/patterns/reliably-run-critical-workflows?ref=patterns)
