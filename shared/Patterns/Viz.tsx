...
// editorial diagram on a dark figure canvas (intentional in both themes, like
// other technical diagrams in the docs). Ported from the patterns design
// handoff (viz.jsx). Only the sections our content uses are included.
...
function VizFrame({
  ...
  return (
    <div className="viz-frame" style={{ "--accent": accent } as React.CSSProperties}>
      <svg
...
// ─── Durable Workflows: vertical trace, retry branch ───────────────────────
function VizDurable({ accent }: { accent: string }) {
  const steps = [
...
// ─── Flow Control: concurrency lanes, throttle marker ───────────────────────
function VizFlow({ accent }: { accent: string }) {
  ...
  const blockH = 22;
  return (
    <VizFrame accent={accent} label="concurrency" sublabel="3 lanes · 12 rps throttle">
      <line
        x1={250}
        y1={20}
        x2={250}
...
// ─── Event Coordination: fan-out from one event ────────────────────────────
function VizEvents({ accent }: { accent: string }) {
  ...
  return (
    <VizFrame accent={accent} label="event" sublabel="user.signed_up · 4 listeners">
      <defs>
        <marker
          id="ev-arrow"
          viewBox="0 0 10 10"
      ...
      <circle cx={src.x} cy={src.y} r="22" fill={accent} fillOpacity="0.15" stroke={accent} strokeWidth="1.25" />
...
// ─── Scheduling: horizontal timeline with cron ticks + sleep arc ─────────────
function VizSchedule({ accent }: { accent: string }) {
  const y = 160;
  const start = 30;
  const end = 450;
  ...
  const sleepTo = 330;
  return (
    <VizFrame accent={accent} label="timeline" sublabel="cron · 0 9 * * * + sleepUntil">
      ...
      {ticks.map((t, i) => (
        <g key={i}>
          <line x1={t} y1={y - 14} x2={t} y2={y + 14} stroke={accent} strokeOpacity="0.7" strokeWidth="1.25" />
...
function VizJobs({ accent }: { accent: string }) {
  ...
  const scale = 5.5;
  return (
    <VizFrame accent={accent} label="queue" sublabel="background workers · 24 active">
      <line
        x1={startX - 4}
        y1={baseY}
        ...
        strokeWidth="1"
      />
      <text x={startX} y={40} fontFamily="Geist Mono, monospace" fontSize="9" fill="#6f665d" letterSpacing="1">
        REQUEST  ───────────►
      </text>
      <text x={startX} y={64} fontFamily="Geist Mono, monospace" fontSize="9" fill={accent} letterSpacing="1">
        BACKGROUND ────►
      </text>
      {depths.map((d, i) => {
        const h = d * scale;
        ...
        return (
          <g key={i}>
            ...
            />
            {d > 18 ? (
              <rect x={x} y={baseY - h - 6} width={barW} height={4} rx="1" fill={accent} fillOpacity="0.5" />
            ) : null}
          </g>
        );
      })}
      <text x={startX} y={baseY + 18} fontFamily="Geist Mono, monospace" fontSize="9" fill="#6f665d" letterSpacing="0.5">
        t-3m
      </text>
      <text
...
const VIZ_BY_ID: Record<string, React.FC<{ accent: string }>> = {
  durable: VizDurable,
  flow: VizFlow,
  events: VizEvents,
...
export default function Viz({ id, accent }: { id: string; accent: string }) {
