...
// Legacy /customers index, ported from pages/customers/index.tsx for the
// flag-off branch. Renders content only — app/layout.tsx provides the
// (legacy) site chrome.
...
const gridItems: GridItem[] = [...grid];
const insertAt = [1, 6, 11, 16, 21];
for (const quote of gridQuotes) {
  gridItems.splice(insertAt.shift()!, 0, quote);
}

export default function CustomersLegacy() {
  return (
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-4 mt-20 grid gap-4 md:mb-6 md:grid-cols-3 md:gap-6">
          {featuredCompanies.map(({ src, name, scale = 1, url }, idx) => (
            <div
              key={idx}
              className="group relative flex flex-col items-center justify-items-start rounded-2xl border border-subtle bg-canvasBase p-6"
            >
          ...
          {gridItems.map(({ type, name, ...item }, idx) => {
            if (type === "quote") {
              ...
              return (
                <blockquote
                  key={idx}
                  className="col-span-2 row-span-2 mx-auto my-8 flex max-w-3xl flex-col gap-8 bg-[url(/assets/textures/wave.svg)] bg-contain bg-center bg-no-repeat px-8 sm:col-span-1"
                >
            ...
            return (
              <div
                key={idx}
                className="group relative flex h-full min-h-[148px] items-center rounded-2xl border border-subtle bg-canvasBase px-6 py-8"
              >
                {!!src && (
                  <Image
                    src={src}
...
function VisitWebsite({ url }: { url: string }) {
  return (
    <a
      href={url}
      target="_blank"
