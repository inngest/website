...
- Runs over the limit are enqueued and execute when capacity is available.
...
export const syncOrderToFulfillment = inngest.createFunction(
  ...
  async ({ event, step }) => {
    // event contains the *last* event after the debounce window closes,
    // so we're always working with the most recent data.

    // Fetch the current order state, not the potentially stale event payload.
