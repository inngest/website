Event-driven systems allow you to have multiple subscribers for a single event stream.  This lets you [fan-out](https://en.wikipedia.org/wiki/Fan-out_(software)#Message-oriented_middleware) an event to multiple functions in parallel.  Running in parallel is useful for separating logic into independent functions, and ensuring that each function retries independently on failure.
...
const funcA = createFunction("A", "user/signup", ({ event }) => { /* Your logic runs on signup here */ });
const funcB = createFunction("B", "user/signup", ({ event }) => { /* And this function runs in parallel on signup */ });
const funcC = createFunction("C", "user/signup", ({ event }) => { /* This function _also_ runs at the same time */ });
```
