Event-driven systems allow you to have multiple subscribers for a single event stream.  This lets you [fan-out](https://en.wikipedia.org/wiki/Fan-out_(software)#Message-oriented_middleware) an event to multiple functions in parallel.  Fan-out is useful for decoupling distinct logic into independently run jobs.
...
const funcA = inngest.createFunction(
  { name: "A" },
  { event: "user/signup" },
  async ({ event }) => { /* Your logic runs on signup here */ },
);
const funcB = inngest.createFunction(
  { name: "B" },
  { event: "user/signup" },
  async ({ event }) => { /* And this function runs in parallel on signup */ },
);
const funcC = inngest.createFunction(
  { name: "C" },
  { event: "user/signup" },
  async ({ event }) => { /* This function _also_ runs at the same time */ },
);
```
