...
2. Create an HTTP handler for a webhook endpoint that verifies [the webhook payload signature](https://docs.stripe.com/webhooks#verify-events) and pushes webhook events to a particular queue
...
* Ensure your worker code is _idempotent_, meaning that if the handler runs multiple times it produces the same side effects and results once
...
Functions can then be [deployed to several supported platforms](/docs/platform/deployment?ref=patterns).

## Additional Resources

* [Blog: Building Webhooks That Scale](/blog/building-webhooks-that-scale?ref=patterns)
* [Docs: Writing Functions](/docs/features/inngest-functions?ref=patterns)
