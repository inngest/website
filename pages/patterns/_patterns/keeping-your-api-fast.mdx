...
app.post("/api/connectSource", await (req, res) => {
...
Functions are defined by declaring what event(s) should trigger it. Any returned value is logged and any error thrown will inform Inngest to [retry to the function](/docs/reference/functions/retries?ref=patterns).

```js
import { createFunction } from "inngest";

createFunction("Backfill User Data", "api/source.connected", async ({ event }) => {
