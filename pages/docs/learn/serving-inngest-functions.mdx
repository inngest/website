...
Inngest executes your functions by calling them directly, either over HTTP or WebSocket. You don't run worker processes or poll a queue. Instead, you expose your functions to Inngest through a connection from your app, and Inngest's execution engine handles invoking them, retrying failures, and managing state.
...
<Callout variant="warn">
  **Using TypeScript SDK v4?** The default mode changed from dev to cloud in v4. When developing locally, set the `INNGEST_DEV=1` environment variable or your app will attempt to connect to Inngest Cloud and fail with a signing key error. [Learn more about local development](/docs/local-development).
</Callout>
...
To securely communicate with Inngest Cloud, your app needs a [signing key](/docs/platform/signing-keys). Set the [`INNGEST_SIGNING_KEY`](/docs/sdk/environment-variables#inngest-signing-key) environment variable in your hosting provider or `.env` file. You can find your signing key in the [Inngest dashboard](https://app.inngest.com).

If you can't use an environment variable, you can pass the signing key directly to the Inngest client constructor via the `signingKey` option. In TypeScript SDK v4, the signing key is configured on the client (not in the `serve()` options). [Read the reference for more information](/docs/reference/serve#serve-client-functions-options).

<Callout variant="info">
  You do not need a signing key for local development. When `INNGEST_DEV=1` is set, the SDK skips signature verification and connects to your local Dev Server.
</Callout>
