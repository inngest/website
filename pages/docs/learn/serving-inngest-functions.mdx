...
SvelteKit can also stream responses back to Inngest, potentially allowing much longer timeouts.

Use `streaming: "allow"` when the SDK can detect that your deployment supports streaming, such as on Cloudflare Pages. Use `streaming: "force"` only when you know your adapter supports streaming but the SDK cannot detect it automatically, such as when deploying SvelteKit to Vercel.

<CodeGroup>
```ts {{ title: "Auto-detect support" }}
import { functions, inngest } from '$lib/inngest';
import { serve } from 'inngest/sveltekit';
...
use an environment variable, you can pass the signing key directly to the Inngest client constructor via the `signingKey` option. [Read
the reference for more information](/docs/reference/typescript/v4/serve#reference).
...
For more information about the `serve` handler, read the [the reference guide](/docs/reference/typescript/v4/serve), which includes:

* [`serve()` configuration options](/docs/reference/typescript/v4/serve#serve-client-functions-options)
* [How the serve handler works](/docs/reference/typescript/v4/serve#how-the-serve-api-handler-works)
