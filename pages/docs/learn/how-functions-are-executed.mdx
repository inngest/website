export const description = "Learn how Inngest works and it's approach to Durable Execution."
...
Steps executed after the first step leverage the state of previous executions and memoization. Here's how it works:
...
14. Depending on the number of attempts configured for the function, the function may be retried (see: [Error handling](/docs/guides/error-handling)):
    ...
    * If the function _has_ exhausted the number of attempts, the function is re-executed with the error thrown. The function can then catch and handle the error as desired (see: [Handling a failing step](https://www.inngest.com/docs/guides/error-handling#handling-a-failing-step)).


{/* TODO - Add what happens when steps are re-ordered (or added) */}
{/* TODO - Add what happens when a step is removed */}
