...
The first argument of every Inngest step method is an `id`. Each step is treated as a separate job which can be individually retried, debugged, or recovered. Inngest uses the ID to memoize step state across function versions.
...
The ID is also used as an identifier in the [Inngest Dev Server](/docs/local-development) for you to quickly find a specific step.
