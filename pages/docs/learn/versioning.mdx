...
It's inevitable that long-running functions change over time. Inngest allows developers to implement multiple strategies for changing long-running code over time. In order to understand how to best manage changes over time, it's important to understand how the SDK implements determinism and [executes steps](/docs/learn/how-functions-are-executed).

## Determinism in functions

Determinism is the same in *every* Inngest language SDK. Except for language-specific idioms, all SDKs implement the same logic. In all SDKs, functions in Inngest are a series of steps. Each step runs reliably, will retry on failure, and is as close to exactly once as possible (outbound network failure when reporting completing steps notwithstanding).
...
1. Hashing the step's unique identifier along with an counter of the number of times the step has been called. This allows steps to be used in a loop.
...
- **Adding new steps to a function is generally safe.** New steps will be executed when the functions re-run (after a step completes). If a function has step `[A, B, C]` and you add a new step `Z` in-between the first steps, the executor will run steps `[A, Z, B, C]` and log a warning. The caveat here is that you must take care to ensure that the new step can run out-of-order and doesn't reference undefined variables. Note that step `B` and `C` will *not* automatically re-run. Instead, a warning will be logged by default. You can change logging a warning and instead permanently fail by enabling strict mode. Permanently failing runs is acceptable, and you can use replay to bulk-replay permanent failures.
- **Forcing steps to re-run by changing step IDs.** This changes the hash, which forces re-evaluation as the step's state is not found. Note that the SDK will log a warning by default as the order of step execution changes. If you change step `C`'s ID to `Z`, your run's state will expect steps `[A, B, C]` to run and instead will see `[A, B, Z]`.
