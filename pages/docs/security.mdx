...
Firstly, it's important to understand that in production all communication between Inngest and your servers is encrypted via TLS. The SDK also actively mitigates attacks by use of the [signing key](/docs/sdk/serve#signing-key), a secret pre-shared key unique to each environment. By default, the signing key adds the following:
...
💡 Signing key rotation is in closed beta. If you'd like access, please email support@inngest.com.
</Callout>

Signing key rotation is a security best practice that helps mitigate the risk of a compromised key. Inngest supports zero downtime signing key rotation if your SDK version meets the minimum version:
...
You can still rotate your signing key if you're on an older version, but you'll experience downtime.

To begin the rotation process, navigate to https://app.inngest.com/env/production/manage/signing-key. Click the "Create new signing key" button and then follow the instructions in the **Rotate key** section.

<Callout variant="info">
🤔 **Why do I need a "fallback" signing key?**

Inngest only allows 1 active signing key at a time. To ensure zero downtime, our SDKs will retry auth failures with the fallback key.
</Callout>

If you use our Vercel integration, note that environment variable setting is manual for signing key rotation. While we automatically created your environment variables when you enabled the integration, the signing key process is manual.
