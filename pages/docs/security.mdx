...
- **Authentication:** requests to your endpoint are authenticated, ensuring that they originate from Inngest. Without knowledge of the signing key, requests are immediately rejected.
...
Within Inngest, you write and serve functions on your own infrastructure using our SDKs. In order to run your functions, they must be synced, or registered, with your Inngest account. Without this, we won't know which functions you're serving or how to run them. Syncing functions is done via a secure handshake. Here's how the handshake works:
1. After your SDK's endpoint is live, a PUT request to the endpoint initiates a secure handshake to the Inngest servers.
