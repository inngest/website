...
For building a full ReAct-style agent loop where each LLM call and tool execution is a retriable step, see the [agent tool loop pattern](/docs/ai-patterns/agent-tool-loops). To expand your system with [sub-agent delegation](/docs/ai-patterns/sub-agent-delegation), use `step.invoke()` and `step.sendEvent()`.
...
- When using `step.ai.wrap`, you can [edit prompts and rerun from the step](/docs/platform/manage/rerun-function-runs?ref=docs-step-ai-orchestration#rerun-from-a-step) in the Dev Server or Inngest Cloud. Arguments must be JSON serializable.
