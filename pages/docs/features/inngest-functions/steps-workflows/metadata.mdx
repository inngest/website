...
Metadata lets you attach custom key-value data to function runs. This data is visible in the Inngest dashboard traces view, useful for tracking processing status, recording business-level metrics, or annotating runs with contextual information. This page covers the API surface. For practical usage examples, see the [metadata how-to guide](/docs/features/inngest-functions/steps-workflows/step-metadata-how-to).
...
A convenience wrapper that creates its own memoized step to attach metadata to the current run. Equivalent to calling `inngest.metadata` inside a `step.run()`. Because it creates a step, it **cannot** be called inside another step.
...
Inngest automatically attaches system metadata when applicable. These are read-only.
