...
When you compare approaches with [`group.experiment()`](/docs/patterns/ai-evals/run-experiments-in-production), use `inngest.score.experiment()` with the returned `experimentRef` to attach a score to the variant that ran. Inngest records it against the experiment so you can compare variants side by side.
