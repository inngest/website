...
## Score an experiment variant

When you compare approaches with [`group.experiment()`](/docs/features/inngest-functions/steps-workflows/running-experiments), use `inngest.score.experiment()` with the returned `experimentRef` to attach a score to the variant that ran. Inngest records it against the experiment so you can compare variants side by side.
...
const quality = await step.run("grade-summary", () => gradeSummary(result));
...
Call `inngest.score.experiment()` at the function-body level, outside any `step.run()`, so the score attaches to the run and its experiment.
