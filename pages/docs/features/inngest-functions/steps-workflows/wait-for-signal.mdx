...
run at a time - via an API call, instead of via an event.

We only recommend this instead of [wait for event](/docs/features/inngest-functions/steps-workflows/wait-for-event) in very specific scenarios:

* Wait for signal is transactional and has lower latency
