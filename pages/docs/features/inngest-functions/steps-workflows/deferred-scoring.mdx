...
To score an [experiment](/docs/patterns/ai-evals/run-experiments-in-production) from a deferred scorer, pass the `experimentRef` from `group.experiment()` to `defer()`. The scorer's returned `{ name, value }` is attached to the variant that ran, with no extra call inside the scorer.
