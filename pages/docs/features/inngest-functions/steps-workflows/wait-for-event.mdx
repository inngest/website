...
This is a useful pattern to react to specific user actions (for example, implement [Human in the loop](/docs/ai-patterns/human-in-the-loop) in AI Agent workflows), or to wait until something happens in external systems.
...
Check out the [`step.waitForEvent()` TypeScript reference.](/docs/reference/typescript/v4/functions/step-wait-for-event)

To add a simple time based delay to your code, use [`step.sleep()`](/docs/reference/typescript/v4/functions/step-sleep) instead.
