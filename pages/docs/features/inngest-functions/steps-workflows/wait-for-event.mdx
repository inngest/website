...
The Steps API enable Inngest Functins to wait for pause until a given event arrives.

This is a useful pattern to synchronize workflows with specific user actions or to implement "Human in the loop" in AI Agent workflows.
...
<GuideSection show="go">
  ...
  Check out the [`step.WaitForEvent()` Go reference.](https://pkg.go.dev/github.com/inngest/inngestgo@v0.7.2/step#WaitForEvent)
...
The "Wait for event" Steps method listens for new events. This means that events triggered before the Function get deployed or started
will not be "catched" by your Step.
