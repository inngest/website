...
* `sessions` groups related function runs in the dashboard. For example, `"sessions": { "conversation_id": "conv_1234" }`. Learn more about [sessions](/docs/features/events-triggers/sessions).
...
type EventPayload = {
  ...
  ts?: number;
  v?: string;
  sessions?: Record<string, string | number | boolean>;
}
```
