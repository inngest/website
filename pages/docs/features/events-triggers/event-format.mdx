...
export const description = "Learn the required and optional fields in an Inngest event payload, including name, data, id, ts, v and meta.sessions. Includes examples and validation rules for well-formed events."
...
* The `name` is the type or category of event. Event `name`s are used to [trigger functions](/docs/learn/inngest-functions). For example, `app/user.created` or `billing/invoice.paid`. See [tips for event naming](#tips-for-event-naming) below.
...
* `id` is a unique identifier for the event used to prevent duplicate events. Learn more about [deduplication](/docs/events#deduplication).
...
* `meta.sessions` groups related function runs in the dashboard. For example, `"meta": { "sessions": { "conversation_id": "conv_1234" } }`. Learn more about [sessions](/docs/features/events-triggers/sessions).
...
<Info>
  While the `user` property is still supported for including user-specific context with the event, it's recommended to instead use the `data` field with [encryption middleware](/docs/features/middleware/encryption-middleware) if you have the need to pass personally identifiable information (PII). Encryption middleware also has added benefits as it supports full or partial encryption of data returned from steps as well as complete control over the encryption key.
</Info>
