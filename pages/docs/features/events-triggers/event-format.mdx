...
export const description = "Learn the required and optional fields in an Inngest event payload, including name, data, id, ts, v. Includes examples and validation rules for well-formed events."
...
* `meta.sessions` groups related function runs in the dashboard. For example, `"meta": { "sessions": { "conversation_id": "conv_1234" } }`. Learn more about [sessions](/docs/features/events-triggers/sessions).
  ...
  {/*
  Add this one to the bottom of EventPayload 
  <Unreleased label="score">
  meta?: {
