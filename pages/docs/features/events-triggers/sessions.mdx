...
<Callout>
  Sessions are for high-cardinality identifiers — one ID per conversation, ticket, or job. They are not for low-cardinality labels like `environment: prod`. For label-style filtering, use [Insights](/docs/platform/monitor/insights) instead.
</Callout>
