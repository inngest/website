...
export const description = "Group related function runs by adding meta sessions to events.";
...
Sessions do not change which functions run. They add event meta that makes related runs easier to find and inspect.
...
- For batched functions, one run can be triggered by multiple events. In that case, Inngest combines the sessions from every event in the batch and keeps up to 25 unique session key/ID pairs for the run.
...
Sessions use the same indexed event and run data that powers [Insights](/docs/platform/monitor/insights). This index is designed for search, filtering, and investigation across activity that Inngest has recorded.
