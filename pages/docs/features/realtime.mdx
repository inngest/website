...
import { Callout, Info, Card, CardGroup, CodeGroup, VersionBadge } from "src/shared/Docs/mdx";
...
For convenience, we also provide a `Realtime.Token` type helper to help type
...
- `bufferInterval?: number` - If set and above `0`, the outputs will only update
  ...
  a UI.
- `token?: Realtime.Subscribe.Token` - The token to be used for subscribing.
