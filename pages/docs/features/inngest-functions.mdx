...
<CardGroup cols={3}>
  <Card title="Triggers" icon={<EventIcon className="text-basis"/>} href={'/docs/features/events-triggers'}>
    A list of Events, Cron schedules or webhook events that trigger Function runs.
  </Card>
  <Card title="Flow Control" icon={<RiGitPullRequestFill className="rotate-90 text-basis" />} href={'/docs/guides/flow-control'}>
    Control how Function runs get distributed in time with Concurrency, Throttling and more.
  </Card>
  <Card title="Steps" icon={<RiGuideFill className="text-basis"/>} href={'/docs/features/inngest-functions/steps-workflows'}>
...
<CardGroup cols={2}>
  <Card title="Background jobs" icon={<RiGuideFill className="text-basis"/>} href={'/docs/guides/multi-step-functions'}>
    Run long-running tasks  out of the critical path of a request.
  </Card>
  <Card title="Delayed Functions" icon={<RiCalendarFill className="text-basis"/>} href={'/docs/learn/how-functions-are-executed'}>
    Schedule Functions that run in the future.
  </Card>
  <Card title="Cron Functions" icon={<RiTimeFill className="text-basis"/>} href={'/docs/guides/scheduled-functions'}>
    Build Inngest Functions as CRONs.
  </Card>
  <Card title="Workflows" icon={<RiMistFill className="text-basis"/>} href={'/docs/features/inngest-functions/steps-workflows'}>
...
<CardGroup cols={1}>
  <Card title="How Functions are executed" icon={<RiMistFill className="text-basis"/>} href={'/docs/learn/how-functions-are-executed'}>
    A deep dive into Inngest's Durable Execution Engine with a step-by-step workflow run example.
  </Card>
  <Card title="Thinking in Steps" icon={<RiGuideFill className="text-basis"/>} href={'/docs/guides/multi-step-functions'}>
