...
To start, make sure you have [installed the Inngest SDK](/docs).
...
1. Automatically run any [functions](/docs/learn/inngest-functions) that are triggered by this specific event, passing the event payload to the function's arguments.
...
<Note>
  When using the [dev server](/docs/local-development), use `http://localhost:8288/e/<fake-key>` as the endpoint. If [self-hosting](/docs/self-hosting), replace with the url for your self-hosted instance.
</Note>
...
* [TypeScript SDK Reference: Send events](/docs/reference/typescript/v4/events/send)
