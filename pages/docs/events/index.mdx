...
Then, sending an event is simple, just call the the `send()` method with your event payload ([Read about the payload format](/docs/reference/events/send#inngest-send-event-payload-event-payload-promise)):
...
💡 Deduplication is valid for 24 hours.
</Callout>
...
For a full reference of how you can send events, read [the reference](/docs/reference/events/send).