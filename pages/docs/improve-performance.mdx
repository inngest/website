...
The Inngest execution model that you are used to is more of a complete handoff to Inngest. Inngest orchestrates functions and through execution of steps and can be bottlenecked on speed due to network roundtrips.

Checkpointing uses the SDK to orchestrate steps and run them locally resulting in immediate execution of subsequent steps in a function. As steps execute, checkpointing 'messages' are passed to Inngest to keep track of progress.
...
Checkpointing flips this: the SDK orchestrates steps locally and executes them immediately. As steps complete, checkpoint messages are sent to Inngest to track progress. The result is dramatically lower latency — ideal for real-time AI workflows.
