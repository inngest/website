...
Checkpointing is enabled by default. No additional configuration is required for long-running or always-on servers.

For serverless platforms, configure `maxRuntime` to slightly below your function's maximum duration:
...
To disable checkpointing, set `checkpointing: false` on the client or per-function.
...
Read [the detailed checkpointing guide](/docs/setup/checkpointing) for more information.
