...
TypeScript will also enforce your custom events being the right shape - see [Event Format](/docs/reference/typescript/v4/events/send) for more details.
...
import { inngest, userNewSignup } from "../../inngest/client";
...
When writing step functions, you can use `waitForEvent` to pause the current function until another event is received or the timeout expires - whichever happens first. Passing an `EventType` variable as the `event` option returns a fully-typed event payload:
...
import { inngest, userNewSignup, userAccountSetupCompleted } from "./client";
...
export default inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const accountSetupCompleted = await step.waitForEvent(
      ...
      {
        event: userAccountSetupCompleted,
        timeout: "2d",
        match: "data.user_id",
      }
    );
