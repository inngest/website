...
The Function Status chart provides a snapshot of the number of finished Function runs grouped by status.
...
You can leverage this chart to identify a possible recurrence or surge in failures and quickly access the Function runs details from the "View all" button.
...
The _Total runs throughput_ is a line chart featuring the **total number of Function runs per App**. This shows the performance of the system of how fast new runs are created and are being handled.
...
The _Total steps throughput_ chart represents **Steps activity at a given point in time, grouped by the selected Apps**.
