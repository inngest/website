...
If the failure was temporary or you have deployed a fix, use the **Rerun** button at the top right of the screen. You can also select a step in the trace to rerun from that step and replace supported step inputs. See [Rerun function runs](/docs/platform/manage/rerun-function-runs?ref=docs-inspecting-function-runs) for the dashboard, Dev Server, CLI, and REST API workflows.
