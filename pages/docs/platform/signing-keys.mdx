...
The TypeScript SDK defaults to cloud mode, which requires a signing key. To disable signature verification for local development, set `INNGEST_DEV=1` in your environment or pass `isDev: true` to the client. This is also useful when running in an automated testing environment.
...
* [TypeScript SDK serve reference](/docs/reference/typescript/v4/serve#signingKey)
