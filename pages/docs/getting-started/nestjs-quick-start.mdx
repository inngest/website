...
In this tutorial you will add Inngest to a [NestJS](https://nestjs.com/) app to easily run background tasks and build complex workflows.
...
In this tutorial you can use any existing NestJS project, or you can create a new one.

<details>
  <summary><strong>Instructions for creating a new NestJS project</strong></summary>
...
Start your NestJS app in development mode:
...
NestJS is built on top of Express, so you can use the `inngest/express` serve handler. Register it in your `main.ts` bootstrap function using `app.use()`:
...
<Callout>
👉 NestJS uses the Express serve handler (`inngest/express`) under the hood. You can swap to any other framework adapter without changing your Inngest function code.
</Callout>
...
With your NestJS server and Inngest Dev Server running, open the Inngest Dev Server UI and select the "Functions" tab [`http://localhost:8288/functions`](http://localhost:8288/functions). You should see your function. (Note: if you don't see any function, select the "Apps" tab to troubleshoot)
...
You will now send an event from within your NestJS app. Add a new `GET /api/hello` endpoint to your `AppController`:

```ts {{ filename: "src/app.controller.ts" }}
...
Every time this API route is requested, an event is sent to Inngest. To test it, open [`http://localhost:3000/api/hello`](http://localhost:3000/api/hello) (change your port if your NestJS app is running elsewhere). You should see the following output: `{"message":"Event sent!"}`
