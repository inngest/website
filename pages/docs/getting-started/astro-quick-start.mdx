...
In this tutorial you will add Inngest to an [Astro](https://astro.build/) app to easily run background tasks and build complex workflows.
...
In this tutorial you can use any existing Astro project with [server-side rendering (SSR)](https://docs.astro.build/en/guides/on-demand-rendering/) enabled, or you can create a new one.

<details>
  <summary><strong>Instructions for creating a new Astro project</strong></summary>
  ...
  Astro requires an [adapter](https://docs.astro.build/en/guides/on-demand-rendering/) for server-side API routes. Install the Node.js adapter:
...
```ts {{ filename: "src/pages/api/inngest.ts" }}
...
With your Astro app and Inngest Dev Server running, open the Inngest Dev Server UI and select the "Functions" tab [`http://localhost:8288/functions`](http://localhost:8288/functions). You should see your function. (Note: if you don't see any function, select the "Apps" tab to troubleshoot)
...
```ts {{ filename: "src/pages/api/hello.ts" }}
...
Every time this API route is requested, an event is sent to Inngest. To test it, open [`http://localhost:4321/api/hello`](http://localhost:4321/api/hello) (change your port if your Astro app is running elsewhere). You should see the following output: `{"message":"Event sent!"}`
