...
Inngest invokes your functions securely via an [API endpoint](/docs/learn/serving-inngest-functions?ref=tanstack-start-quick-start) at `/api/inngest`. To enable that, you will create an [Inngest client](/docs/reference/typescript/v4/client/create?ref=tanstack-start-quick-start) in your TanStack Start project, which you will use to send events and create functions.
...
<Callout>
👉 Note that you can import [`serve()`](/docs/reference/typescript/v4/serve?ref=tanstack-start-quick-start) for other frameworks and the rest of the code, in fact, remains the same — only the import statement changes. TanStack Start uses `inngest/edge` because it runs in an edge runtime environment.
</Callout>
...
- About [Inngest functions](/docs/learn/inngest-functions?ref=tanstack-start-quick-start).
- About [Inngest steps](/docs/learn/inngest-steps?ref=tanstack-start-quick-start).
...
- How to [deploy your app to your platform](/docs/apps/cloud?ref=tanstack-start-quick-start).
