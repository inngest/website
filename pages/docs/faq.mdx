...
- [Why are my functions "stalling" or taking a long time to start?](#why-are-my-functions-stalling-or-taking-a-long-time-to-start)
...
## Why are my functions "stalling" or taking a long time to start?

If your functions appear to "stall" or steps take longer than expected to begin executing, you may be hitting concurrency limits. This is a common issue when testing functions with high parallelism or fan-out patterns.

### In local development

The [Inngest Dev Server](/docs/local-development) defaults to **100 parallel queue workers**. When you exceed this capacity, additional steps must wait in a queue.

**Solution:** Increase the queue workers when starting the Dev Server:
...
Your [account plan](/pricing?ref=faq-stalling) determines your maximum concurrency limit. If you're consistently hitting this limit, steps will queue and wait for capacity.

**Solutions:**
1. Check your concurrency usage in the [metrics dashboard](https://app.inngest.com)
2. Consider [upgrading your plan](/pricing?ref=faq-stalling) for higher limits
3. Review your [concurrency configuration](/docs/guides/concurrency) to optimize how capacity is distributed

For more detailed troubleshooting, see the [concurrency troubleshooting guide](/docs/guides/concurrency#troubleshooting-concurrency-issues).
