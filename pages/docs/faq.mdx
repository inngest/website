...
export const description = "Answers to common Inngest questions about latency, cron configuration, stopping functions, branch environment keys, timeouts, authentication, data redaction, and more."
...
- [Why might my Hobbyist account experience increased latency?](#why-might-my-hobbyist-account-experience-increased-latency)
...
- [How do I specify multiple serve paths for a same Vercel application on the Dashboard?](#how-do-i-specify-multiple-serve-paths-for-a-same-vercel-application-on-the-dashboard)
...
The best way to ensure a deprecated function doesn't run is to deploy without including it in your [serve handler](/docs/reference/typescript/v4/serve). You can temporarily achieved the same result by archiving the function on our dashboard, but note that a new deployment will unarchive the function.

## Why might my Hobbyist account experience increased latency?

After multiple rounds of internal testing, Inngest may introduce major infrastructure changes gradually to small cohorts of Hobbyist accounts before expanding a rollout to Pro and Enterprise. The Hobbyist plan is designed for testing and validating workflows rather than production use, so starting with these accounts reduces the risk of disrupting end users. These infrastructure changes are expected to benefit every account; Hobbyist users receive them first so we can validate behavior that cannot be fully reproduced internally.

Some Hobbyist accounts may experience temporary degraded performance during a rollout while we monitor and resolve issues. If you are evaluating Inngest and need a more predictable environment, [contact our Sales team](/contact?ref=docs-faq). We can help move your evaluation to infrastructure with stronger performance isolation. Upgrading to [any paid plan](/pricing?ref=docs-faq) automatically moves your account to infrastructure isolated from the Hobbyist rollout pool.

Read [how Inngest rolls out infrastructure changes](/blog/how-inngest-rolls-out-infrastructure-changes?ref=docs-faq) for more information.
...
We recommend doing [E2E encryption](/docs/reference/typescript/v4/middleware/examples#e2-e-encryption) instead, as it's more secure and plaintext data never leaves your servers.
...
For more information, see the [Upgrading from v2 to v3](/docs/reference/typescript/v3/migrations/v2-to-v3) migration guide.
...
## Why am I getting an `Invalid signature` error on my runs?
Recent changes in Vercel's runtime surfaced a signature validation issue in older SDK versions. Updating to TypeScript SDK `3.50.0` (or later) resolves this and restores normal run verification behavior.
...
Learn more about this in the [dev server](/docs/local-development#auto-discovery) docs.
