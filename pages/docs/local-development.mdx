...
## Concurrency and queue workers

The Dev Server processes function steps using parallel workers. By default, the Dev Server uses **100 parallel queue workers** which determines how many steps can execute concurrently.

<Callout>
**Tip:** If your functions appear to "stall" or steps take a long time to start executing in local development, you may have reached the Dev Server's concurrency limit. This is especially common when testing functions that trigger many parallel steps or fan-out patterns.
</Callout>

### Configuring queue workers

You can adjust the number of queue workers using the `--queue-workers` flag:

```shell
# Increase queue workers for high-concurrency testing
npx inngest-cli@latest dev --queue-workers 500

# Or reduce for testing concurrency behavior
npx inngest-cli@latest dev --queue-workers 10
```

This is useful when:
- **Testing high-volume scenarios**: Increase workers to process more steps in parallel
- **Simulating production constraints**: Reduce workers to better match your production concurrency limits
- **Debugging concurrency issues**: Lower the worker count to reproduce issues caused by concurrency limits

### Identifying concurrency-related delays

In local development, if you notice:
- Steps waiting longer than expected to start
- Function runs appearing to "hang" between steps
- Large backlogs of pending work

These symptoms may indicate you've hit the Dev Server's queue worker limit. Try increasing the `--queue-workers` value or review your function's [concurrency configuration](/docs/guides/concurrency) to ensure it matches your testing needs.

<Tip>
  Unlike the Inngest Platform (which provides [metrics and observability](/docs/platform/monitor/observability-metrics) for monitoring concurrency), the Dev Server does not currently surface concurrency metrics. Use the `--queue-workers` flag to adjust capacity based on your testing requirements.
</Tip>
...
| --queue-workers | -              | int      | 100                               | Number of workers to execute steps from the queue         |
...
1. Expose the Dev Server ports (`8288`, `8289`).
...
* The `ports` configuration exposes the Dev Server on port `8288` so you can view this on your host machine in the browser. It also exposes `8289` which is used for [`connect()`](/docs/setup/connect).

\* - The TypeScript SDK defaults to cloud mode. Setting `INNGEST_DEV=1` explicitly enables dev mode, which connects to the Dev Server and disables signature verification.
