import { VersionBadge, Callout, CodeGroup } from "src/shared/Docs/mdx";
...
```
```shell {{ title: "yarn" }}
yarn add @inngest/otel
```
```shell {{ title: "pnpm" }}
pnpm add @inngest/otel
```
```shell {{ title: "bun" }}
bun add @inngest/otel
```
</CodeGroup>
...
Many frameworks start their dev server through a CLI, so there is no direct way to pass `--import` to Node. TanStack Start and Nuxt, for example, run `vite dev`. In development, set the preload with the `NODE_OPTIONS` environment variable instead. It applies to the Node process the CLI starts and propagates to any worker threads the framework uses to run server code.

```json package.json
{
  "scripts": {
    "dev": "NODE_OPTIONS='--import @inngest/otel/node' vite dev"
  }
}
```

In production, these frameworks build a Node server entrypoint, so preload as usual:

```shell
node --import @inngest/otel/node .output/server/index.mjs
```
