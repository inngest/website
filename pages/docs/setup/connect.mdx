...
<Warning>
  The `connect` API is currently in [developer preview](/docs/release-phases#developer-preview) and is not yet recommended for critical production workloads. We recommend deploying to a staging environment first prior to deploying to production.
</Warning>
...
During the Inngest [developer preview](/docs/release-phases#developer-preview), we recommend setting up a new app for trying out `connect`. We will support gradually migrating your existing `serve` apps in a future release.

## Developer preview

The `connect` API is currently in [developer preview](/docs/release-phases#developer-preview). This means that the API is not yet recommended for critical production workloads and is subject to breaking changes.
