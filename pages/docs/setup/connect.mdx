...
When deploying a new version of your application either by changing the function configuration or updating the app version, the first instance to reach `connect()` will kick off a sync during the connection setup process. This will update function configurations to the desired state configured in your code.
