...
<Info>
  When a worker reaches its `maxWorkerConcurrency` limit, you may see an **"All workers are at capacity"** error in the Inngest dashboard. This is a retryable error and no action is required — Inngest will automatically retry and route work to a worker with available capacity or wait for a worker to become available.
</Info>
