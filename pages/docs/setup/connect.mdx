...
<Warning>
  The `connect` API is currently in developer preview and is not yet recommended for critical production workloads. We recommend deploying to a staging environment first and reaching out to the Inngest team prior to deploying to production.
</Warning>
...
<Warning>
  As this is a developer preview, the `connect` API may be subject to breaking changes.
</Warning>