...
If you have multiple connections from the same worker, but with different `instanceId` values, they can each specify a different `maxWorkerConcurrency` limits.
