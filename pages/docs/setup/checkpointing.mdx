...
export const myFunction = inngest.createFunction(
  ...
  { event: "app/my.event" },
  async ({ step }) => {
    // This function uses checkpointing
  }
);
```
...
Checkpointing is currently in developer preview. This means the API is subject to change as we iterate based on feedback. Read the [release phases](/docs/release-phases) for more details.
