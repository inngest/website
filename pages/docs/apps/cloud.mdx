...
- Apps are synced to one environment. The [**`INNGEST_SIGNING_KEY`**](/docs/sdk/environment-variables#inngest-signing-key) ensures that your app is synced within the correct Inngest environment. Verify that you assigned your signing key to the right `INNGEST_SIGNING_KEY` environment variable in your hosting provider or **`.env`** file locally.
