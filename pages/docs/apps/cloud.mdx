...
Use the [Inngest REST API](https://api-docs.inngest.com/v2/apps/SyncApp) to programmatically sync your app. This is particularly useful when syncing from a CI/CD pipeline.

Before you send the request, you'll need 3 things:
1. An [API key](/docs/platform/api-keys).
2. The URL for your app's Inngest endpoint (e.g. `https://my-app.com/api/inngest`).
3. Your app's ID (this is the Inngest client's ID).

Once you have these, you can send a request like this:
...
<details>
  ...
  1. **Different App ID:**
  - If you resync the app after modifying the [app ID](/docs/reference/typescript/v4/client/create), a new app is created, not a new sync within the existing app.
