...
export const metaTitle = "Inngest Dev Server MCP | Local AI Development Tools";
export const description = "Connect AI coding agents to your local Inngest Dev Server via MCP. List functions, send events, invoke functions, and inspect local runs.";

# Inngest Dev Server MCP

The Inngest Dev Server includes a local Model Context Protocol (MCP) server that gives AI assistants direct access to the functions, events, and runs on your machine. Use it to test, debug, and build Inngest applications without connecting to Inngest Cloud.

<Callout variant="info">
Looking for deployed apps and runs? [Inngest Cloud MCP](/docs/ai-dev-tools/cloud-mcp) is a separate, hosted, read-only server that connects to your Inngest Cloud account with an API key.
</Callout>

## What is MCP?

[Model Context Protocol (MCP)](https://modelcontextprotocol.io/introduction) is a standard for connecting AI assistants to external tools and data sources. Dev Server MCP provides AI tools like Claude Code and Cursor with:
...
<Callout>
Dev Server MCP runs locally with your dev server and requires no external dependencies, API keys, or internet connection to function.
</Callout>
...
Dev Server MCP is automatically available when you start the Inngest dev server:
...
Dev Server MCP provides 8 tools organized into three categories:
