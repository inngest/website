...
export const description =
  "Use Inngest with AI coding agents, Cloud and Dev Server MCP, CLI workflows, and LLM-ready docs to build, test, and debug durable functions faster.";

# AI development tools for Inngest

Use Inngest with AI coding agents, hosted and local MCP tools, CLI workflows, and LLM-ready documentation. These resources help agents write current Inngest code, inspect deployed apps and runs, connect to your local Dev Server, trigger test events, and debug durable functions in one loop.

<CardGroup cols={2}>
  <Card
    href={"/docs/ai-dev-tools/cloud-mcp?ref=docs-ai-dev-tools"}
    title={"Cloud MCP"}
    icon={<MCPIcon className="text-basis h-12 w-12" />}
    iconPlacement="top"
  >
    Connect AI assistants to Inngest Cloud to inspect environments, apps, functions, events, runs, and traces with read-only tools.
  </Card>
...
- **Cloud MCP** gives your agent read-only access to deployed apps, functions, events, runs, and traces in Inngest Cloud.
...
Use [Cloud MCP](/docs/ai-dev-tools/cloud-mcp?ref=docs-ai-dev-tools) to investigate deployed apps and runs. For local development, install the agent plugin or skills, start the Inngest Dev Server, and connect the tool to [Dev Server MCP](/docs/ai-dev-tools/mcp?ref=docs-ai-dev-tools).
