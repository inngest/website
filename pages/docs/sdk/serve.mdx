...
Inngest provides a `serve()` handler which adds an API endpoint to your router. To make automated deploys much easier, **the endpoint needs to be defined at `/api/inngest`** (though you can [change the API path](/docs/reference/serve#serve-client-functions-options)).
...
See the [Astro example](https://github.com/inngest/inngest-js/tree/main/examples/bun) for more information.
