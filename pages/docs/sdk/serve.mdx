...
Firebase Cloud Function requires to configure the `INNGEST_SERVE_PATH` with the custom function path.
...
You can now server your Firebase Cloud Function locally using the following command:

```bash
firebase emulators:start
```

Please note that you'll need to start your Inngest Local Dev Server with the `-u` flag as follows:
