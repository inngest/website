...
- [Inngest clients and functions now require IDs](#clients-and-functions-require-ids) - `name` was often being slugified automatically, causing some confusion around if it was safe to change a client/function's display name.
- [Steps now require IDs](#all-steps-require-ids) - Like `step.run()`, all steps now require a string to help identify what they're for.
- [Serve handlers refactored](#serve-handlers-refactor) - Creating a custom serve handler was a bit heavy, so we've lightened the process and made serve handlers much simpler.
- [Shorthand function creation removed](#shorthand-function-creation-removed) - We're removing the ability to create a function without providing an options object, as it hid some important configuration for users.
- [Some environment variables removed](#environment-variables) - We've reorganized `INNGEST_*` environment variables to be a bit more sane following many iterations in v2.
- [Function `fns` option removed](#fns-removed) - We have a better answer for this problem with middleware, so we're removing this direct option.
...
See [Introducing Inngest TypeScript SDK v3.0](#) to see what these features unlock for the future of the TS SDK.
...
<Callout>
We recommend keeping your `name` defined as-is and just adding the new required `id`. This will mean that the name of functions in the dashboard is unchanged.
</Callout>
...
We've removed some environment variables for setting URLs for communicating with Inngest.

- **✅ Added `INNGEST_BASE_URL`** - set the URL to communicate with Inngest in one place
