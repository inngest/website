...
- [INNGEST_SERVE_HOST](#inngest-serve-host) _(deprecated in TS SDK v4)_
...
_This environment variable is replacing the `INNGEST_SERVE_HOST` in the `v4` version of the TypeScript SDK._

The host used to access this application from Inngest Cloud.

If set, it should be the host including the protocol and port, e.g. `http://localhost:8288` or `https://my.tunnel.com`. Can be overwritten by manually specifying `serveOrigin` in `serve()`.
...
_Note that this environment variable still works in all SDKs, but in v4+ of the TypeScript SDK it has been deprecated in favor of `INNGEST_SERVE_ORIGIN`._
...
Can be overwritten by manually specifying `signingKey` on the Inngest client.
