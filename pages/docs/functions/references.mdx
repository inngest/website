...
Using [`step.invoke()`](/docs/reference/functions/step-invoke), you can directly call one Inngest function from another and handle the result. You can use this with `referenceFunction` to call Inngest functions located in other apps.
...
A referenced function can be used with [`step.invoke()`](/docs/reference/functions/step-invoke) to invoke the function and wait for the result.
