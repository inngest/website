...
Middleware can be added in two places by providing an array of middleware: when instantiating an Inngest client via `new Inngest()`, and when creating a function via `inngest.createFunction()`.
