...
We can alter the input arguments for a function run (event data, `step` tooling, etc.), using `run`'s `input()` hook. (See [Middleware - Lifecycle - Hook reference](/docs/reference/middleware/lifecycle#hook-reference) for a list of all available hooks.)
