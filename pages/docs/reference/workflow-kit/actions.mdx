...
In the example above, the `actionsDefinition` array is passed to the [`<Provider />`](/docs/reference/workflow-kit/components-api) while the `actions` are passed to the [`Engine`](/docs/reference/workflow-kit/engine).
