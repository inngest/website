...
- Set to a URL if the Dev Server is not hosted at `http://localhost:8288`. For example, you may need to set it to `http://host.docker.internal:8288` when running the Dev Server within a Docker container. Learn more in our [Docker guide](/docs/guides/development-with-docker). URLs are not supported below version `0.4.6`.
