...
By default, `event.data.encrypted` is encrypted. All other fields are sent in plaintext. If you'd like to encrypt a different field, set the `event_encryption_field` parameter.

## Decrypt only mode

If you'd like to disable encryption but continue decrypting, set `decrypt_only=True`. This is useful when you want to migrate away from encryption but continue processing older events.
