...
Limit the number of times a function runs for a given `key` over a time `period`. All events in excess of the `limit` are ignored and never processed by the function.
...
As `rateLimit` ignores all events in excess of the `limit` during the given `period`, this is not useful if you need to process every single event. Instead, you might be more interested in [configuring a `concurrency` limit with a `key`](/docs/functions/concurrency#limiting-concurrency-for-a-specific-key).

---

## How `rateLimit` works

Each time an event is received that match's your function's trigger, it is evaluated prior to executing your function. If `rateLimit` is configured, Inngest creates a counter of the number of functions that are triggered by events over a given time `period`. The time period is begins when the first event is received and is reset when the period ends. In essence this acts as a mechanism to protect a given function from being run more than desired over a given period.

When a `key` is added, a separate counter is created for each unique value of the `key` expression. For example if your `key` is set to `event.data.customer_id`, each customer would have their own counter to limit the number of functions run.
...
<img
  ...
  height={320}
  quality="95"
  alt="Visualization of the function rate limit"
/>
...
<Properties>
  <Property name="rateLimit" type="object">
    ...
    <Properties nested={true}>
      <Property name="limit" type="integer" required>
...
export default inngest.createFunction(
  ...
  { event: "intercom/company.updated" },
  async ({ event, step }) => {
    const company = await step.run('fetch-latest-company-data-from-intercom', async () => {
...
export default inngest.createFunction(
  {
    name: "Send check failed notification",
    rateLimit: {
