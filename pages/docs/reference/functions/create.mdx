...
<Properties>
  ...
  <Property name="batchEvents" type="object">
    Configure how the function should consume batch of events ([reference](/docs/guides/batching))
    ...
    <Properties nested={true}>
      ...
      <Property name="timeout" type="string" required>
        How long should we wait before invoking the function with the batch even if it's not full.
...
```ts
function handler({ event, events step, runId }) {/* ... */}
```
...
#### `events` <VersionBadge version="v2.1.0+" />

`events` is a list of `event` payload objects that's accessible when the `batchEvents` is set on the function configuration.
Without batching, it's a single item list that has the same object as `event`
