...
Define any failure handlers for your function with the [`onFailure`](/docs/reference/functions/create#configuration) option. This function will be automatically called when your function fails after it's maximum number of retries. You can alternatively, use the [`"inngest/function.failed"`](#the-inngest-function-failed-event) system event to handle failures across all functions.
...
* Perform a rollback of the transaction (i.e. undo the work possibly done in your main function handler)
...
The [`Error`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Error/Error) as thrown from the last retry in your main function handler.
...
This will be the function run ID for the error handling function, not the original function. [Learn more about `runId` here](/docs/reference/functions/create#run-id).
...
<Properties>
  <Property name="name" type={`"inngest.function.failed"`} attribute>
    The `inngest/` event prefix is reserved for system events in your environment.
  </Property>
...
export default inngest.createFunction(
  {
    ...
    // Your handler can be an async function:
