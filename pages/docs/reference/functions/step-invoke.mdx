...
`step.invoke()` is used to call other functions within your event-driven system. It allows you to invoke functions either by specifying the function's event identifier or by passing the function directly. The method returns a `Promise` that resolves with the return value of the invoked function.
...
<Row>
  <Col>
    <Properties>
      ...
      <Property name="options" type="object" required>
        ...
        * `function`: Either the ID or the local instance of the function to invoke.
        * `data`: Optional data to pass to the invoked function.
        * `user`: Optional user context for the invocation.
  ...
  <Col>
    ```ts
    // Invoke a function using its event identifier
    const resultFromEvent = await step.invoke("invoke-by-id", {
...
When passed a function to invoke, internally we retrieve the ID of the function to invoke using the function's ID and the ID of the client used to create it. It can also be passed a string directly, in which case the user is reliant on Inngest to assert that the ID is correct. See [Error handling](#error-handling) for more information around this point.
