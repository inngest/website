...
<Callout>
We recommend setting the `INNGEST_EVENT_KEY` as an environment variable over setting the `eventKey` option. As with any secret, it's not a good practice to hard-code the event key in your codebase.
</Callout>
