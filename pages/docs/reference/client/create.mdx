...
<Properties>
  ...
  <Property name="logLevel" type={`"fatal" | "error" | "warn" | "info" | "debug"`}>
    The minimum log level to output. Defaults to `"info"`. See also [`INNGEST_LOG_LEVEL`](/docs/sdk/environment-variables#inngest-log-level).
  </Property>
...
const accountCreated = eventType({
...
const subscriptionStarted = eventType({
