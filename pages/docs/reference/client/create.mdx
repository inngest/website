...
<Properties>
  ...
  <Property name="logger" type="Logger" version="v2.0.0+">
    A logger object that provides the following interfaces (`.info()`, `.warn()`, `.error()`, `.debug()`). Defaults to using `console` if not provided. See [logging](/docs/reference/client/logging) for more details.
  </Property>
