...
Check out the [Common Actions Middleware Example](/docs/reference/typescript/v3/middleware/examples#common-actions-for-every-function?ref=migration) for the code.
...
- [Middleware](/docs/features/middleware?ref=migration) - specify functions to run at various points in an Inngest client's lifecycle
...
For more information, see [Defining Event Payload Types](/docs/reference/typescript/v3/client/create#defining-event-payload-types?ref=migration).
...
Async step functions with v1 of the Inngest TS SDK unlocks a huge `Array<Possibility>`. To explore these further, check out the [multi-step functions](/docs/learn/inngest-steps?ref=migration) docs.
