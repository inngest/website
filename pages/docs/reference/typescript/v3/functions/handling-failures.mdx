...
The `onFailure` handler function has the same arguments as [the main function handler](/docs/reference/typescript/v3/functions/create#handler) when creating a function, but also receives an `error` argument.
...
[See the `step` reference in the create function documentation](/docs/reference/typescript/v3/functions/create#step).

### `runId`

This will be the function run ID for the error handling function, _not the function that failed_. To get the failed function's run ID, use `event.data.run_id`. [Learn more about `runId` here](/docs/reference/typescript/v3/functions/create#run-id).
