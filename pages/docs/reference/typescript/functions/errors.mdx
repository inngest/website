...
Inngest automatically handles errors and retries for you. You can use standard errors or use including custom error types to control how Inngest handles errors.

Read more about the approaches to handle errors in [our guide](/docs/guides/error-handling).
...
<Properties>
  ...
  <Property name="retryAfter" type="number | string | date" required>
    ...
    * `string` - A time string, parsed by the [ms](https://npm.im/ms) package, e.g. `"30m"`, `"3 hours"`, or `"2.5d"`.
    * `date` - A `Date` object.
  </Property>
  <Property name="options" type="object">
    <Properties nested={true} collapse={true}>
...
After a step exhausts all of it's retries, it will throw a `StepError` which can be caught and handled in the function handler if desired.
