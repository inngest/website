...
<Properties>
  ...
  <Property name="endpointAdapter" type="InngestEndpointAdapter.Like">
    An endpoint adapter that enables [Durable Endpoints](/docs/reference/typescript/v4/durable-endpoints). Import from `inngest/edge` or `inngest/node`. When provided, `inngest.endpoint()` and `inngest.endpointProxy()` become available.
  </Property>
</Properties>
