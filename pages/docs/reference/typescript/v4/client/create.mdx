...
<Properties>
  ...
  <Property name="logger" type="Logger">
    A logger object that provides `.info()`, `.warn()`, `.error()`, and `.debug()` methods. Log level is controlled by the logger instance itself. The SDK ships a built-in `ConsoleLogger` that accepts a minimum log level. Defaults to `new ConsoleLogger("debug")` if not provided. See [logging guide](/docs/guides/logging) for more details.
...
Event types defined with `eventType()` are fully typed when used with `inngest.send()` and `inngest.createFunction()`. This can more easily alert you to issues with your code during compile time.
