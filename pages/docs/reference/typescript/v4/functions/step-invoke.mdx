...
When a function object is passed as an argument, internally, the SDK retrieves the function's ID automatically. For cross-app invocation, use [`referenceFunction()`](/docs/functions/references) to create a typed reference. See [Error handling](#error-handling) for more information.
...
If not explicitly configured, the default timeout for `step.invoke` is 1 year.
