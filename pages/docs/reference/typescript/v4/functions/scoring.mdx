...
<Properties>
  <Property name="id" type="string" required>
    The ID of the step. This is used to memoize the score write so it is not re-sent on replays or retries. Like the ID passed to `step.run()`, it must be unique within the run.
  </Property>
  <Property name="name" type="string" required>
    Score label.
  </Property>
