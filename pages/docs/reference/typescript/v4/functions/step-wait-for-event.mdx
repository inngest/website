...
<Callout variant="warning">
  **Known limitation with `in` clauses:** When using the `if` option with CEL expressions, the `in` operator (for example, `async.data.function_id in ["a", "b"]`) may be silently stripped from the expression under certain conditions, causing your `waitForEvent` to match more broadly than intended. As a workaround, use multiple equality checks with `||` instead:
