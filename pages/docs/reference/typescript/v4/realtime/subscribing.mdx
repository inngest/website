...
<Row>
  <Col>
    ...
    Returns `Promise<Realtime.Subscribe.ClientToken>` with `key` (the JWT string) and `apiBaseUrl` (the resolved server URL). `inngest/react` also exports this shape as the `ClientSubscriptionToken` type alias.
  </Col>
  <Col>
  ```ts
...
<Info>
  A client token does not include `channel` or `topics`. When passing it to
  `useRealtime`, pass the same channel and topics as top-level hook options so
  the hook can build the subscription and preserve typed message inference.
</Info>
