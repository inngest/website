...
<Callout>
  Channel parameters are yours to name and yours to generate. Use a domain-specific
  term within your your product, such as `threadId`, `contentId`, `documentId`, or
  `userId`.
</Callout>
...
await inngest.realtime.publish(ch.message, { text: "Hello!", sender: "alice" });
```
...
type Params = typeof pipeline.$params;             // { contentId: string }
```
