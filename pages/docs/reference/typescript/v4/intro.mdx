...
<Info>
  Inngest is an event-driven durable execution platform that lets you write reliable background jobs, scheduled
  tasks, and multi-step workflows in TypeScript without any additional infrastructure. Simply define functions
  as code; Inngest handles retries, concurrency, rate limiting, and observability.
  ...
  - **Faster by default** — Parallel step optimization and checkpointing are both enabled by default, leading
    to fewer requests and lower latency.
