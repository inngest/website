...
Use this along with [mutating function input](/docs/features/middleware/dependency-injection?guide=typescript#mutating-input) to set a new `ai` property that you can use within functions, like in the following example:
...
inngest.createFunction(
  {
    id: "user-data-dump",
    triggers: { event: "app/data.requested" },
  },
...
While this example uses Prisma, it serves as a good example of using the [onFunctionRun -> input](/docs/reference/typescript/v4/middleware/lifecycle#onfunctionrun-lifecycle) hook to mutate function input to perform crucial setup for your functions and keep them to just business logic.
...
inngest.createFunction(
  {
    id: "example",
    triggers: { event: "app/user.loggedin" },
  },
  async ({ prisma }) => {
...
Check out [Common actions for every function](/docs/reference/typescript/v4/middleware/examples#common-actions-for-every-function) to see how this technique can be used to create steps for all of your unique logic.
