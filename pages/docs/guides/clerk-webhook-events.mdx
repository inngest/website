...
You can make use of the fact that with Inngest, each function has [automatic retries](/docs/features/inngest-functions/error-retries/inngest-errors), so only the code that has issues is re-run.
...
To send a follow-up email, you can use the [`step.run()`](/docs/reference/typescript/v4/functions/step-run). This method will encapsulate specific code that will be automatically retried ensuring that issues with one part of your function don't force the entire function to re-run. Additionally, you will extend the functionality with [`step.sleep()`](/docs/reference/typescript/v4/functions/step-sleep).
...
* Reference: [`step.run()`](/docs/reference/typescript/v4/functions/step-run)
