...
It’s easy to run steps in parallel within a single Inngest function.  You can keep track of of each step’s output and wait for all parallel stops to complete.  There are some benefits to using parallelism within Inngest:

- It uses the standard library.  You don't have to learn another SDK or use platform-specific code.
- You don’t have to keep track of output from parallel steps;  we do that for you
- You can run parallel map-reduce without having to keep track of state 
...
Currently, the total data returned from ******all****** steps must be under 4MB (eg. a single step can return a max of. 4MB, or 4 steps can return a max of 1MB).  Functions are also limited to a maximum of 1,000 steps.
