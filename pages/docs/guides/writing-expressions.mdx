...
* Defining keys for [concurrency](/docs/functions/concurrency), [rate limiting](/docs/reference/functions/rate-limit), or [debounce](/docs/reference/functions/debounce)
...
* **Values** - Other expressions can return any value which might be used as keys (ex. concurrency, rate limit, debounce or idempotency keys) or a dynamic value (ex. run priority). {/* TODO - Link to: /docs/guides/handling-idempotency */}

## Examples

Most expressions are given the `event` payload object as the input. Expressions that match additional events (ex. wait for event, cancellation) will also have the `async` object for the matched event payload. [This is a reference of all the operators available in CEL](https://github.com/google/cel-spec/blob/master/doc/langdef.md#list-of-standard-definitions).
...
* You can use single `'` or double quotes `"` for strings, but we recommend sticking with one
* In the TypeScript SDK, write expressions within backticks `` ` `` to use quotes in your expression or use JavaScript's string interpolation.
* Use ternary operators to return default values
* Unlike in JavaScript, the or operator (`||`) does not return the value of the statement left of the operator if truthy. Use the ternary operator (`?`) instead of `||` for conditional returns.
