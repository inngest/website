...
In the above example, we fetch all users within a single step.  This step will retry on error.  Once that’s complete (via `await`), we create an *array of events* which are sent to Inngest via `step.sendEvent`.

Each event received will trigger a new function run for any function listening to those events, just like regular events do.
