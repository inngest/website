...
The fan-out pattern enables you send a single event and trigger multiple functions in parallel (one-to-many). The key benefits of this approach are:
...
A use case for fan-out is when a user signs up for your product, you may want to:
...
As Inngest is powered by events, implementing fan-out is as defining multiple functions that use the same event trigger. Let's take the above example of user signup and implement it in Inngest.

First, we'll instrument out `/signup` route handler to send an event to Inngest when a user signs up:
...
Now, with this event, any function using `"app/user.signup"` as it's event trigger will be automatically invoked.

Here, we'll define our two functions: `sendWelcomeEmail` and `startStripeTrial`. As you can see below, both functions use the same event trigger, but perform different work.
...
We've now successfully implemented fan-out in our application. Each function will run independently and in parallel. If one function fails, the other will not be disrupted.

Other benefits of fan-out include:

* **Bulk Replay**: If a third party API goes down for a period of time (e.g. your email provider), you can use [Replay](/docs/platform/replay) to selectively re-run all functions that failed, without having to re-run all sign-up flow functions.
