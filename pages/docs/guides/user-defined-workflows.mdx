...
Once the `workflows` table created, we will add two [workflow instances](/doc/reference/workflow-kit/workflow-instance) records:
...
The `<AutomationEditor>` component is instantiated with a [workflow instance](/doc/reference/workflow-kit/workflow-instance) loaded from the database:
