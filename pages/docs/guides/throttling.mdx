...
If your use case requires strict request smoothing, please [contact us][contact].
</Callout>

**Configuration information**

- Using throttle ensures no more than `limit` runs start within the given `period`.
- For each window of `period / limit`, `limit + burst` runs may start. Limit is added by default to ensure high throughput on a non-uniform arrival rate.
