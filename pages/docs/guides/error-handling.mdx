...
Throw a [non-retriable error](/docs/features/inngest-functions/error-retries/inngest-errors#non-retriable-error) when an error is permanent and should bypass remaining retries, such as invalid user input or a missing record that will not become available later.
