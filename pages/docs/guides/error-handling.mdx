...
import { RiArrowGoBackLine, RiErrorWarningFill } from "@remixicon/react";
...
<CardGroup cols={1}>
  ...
  <Card title="Failure handlers" icon={<RiErrorWarningFill />} href={'/docs/features/inngest-functions/error-retries/failure-handlers'}>
    Utilize callbacks to handle all failing retries.
  </Card>
  <Card title="Rollbacks support" icon={<RiArrowGoBackLine />} href={'/docs/features/inngest-functions/error-retries/rollbacks'}>
