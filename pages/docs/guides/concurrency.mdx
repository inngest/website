...
<Callout>
**Important:** Concurrency limits the number of _steps_ executing at a single time, **not** the number of function runs. A function run that is sleeping, waiting for an event, or paused between steps does **not** count against your concurrency limit. Only steps that are actively executing code count toward the limit.
...
Because sleeping or waiting is common, concurrency _does not_ limit the number of functions in progress. Instead, it limits the number of steps executing at any single time.
...
The key insight is that your concurrency limit applies to _active execution_, not to the number of function runs in progress. Consider a function with a `concurrency` limit of `10`:
...
The [Inngest Dev Server](/docs/local-development) uses **100 parallel queue workers** by default. If you're testing functions with high parallelism (such as fan-out patterns), you may exhaust this capacity.

**Solutions:**

1. **Increase queue workers**: Start the Dev Server with more workers:
   ```shell
   npx inngest-cli@latest dev --queue-workers 500
   ```

2. **Review your function design**: If your functions create many parallel steps, consider whether this matches your production requirements

3. **Test with realistic concurrency**: If your production functions have concurrency limits, consider matching those in local development to catch issues early
...
1. **Metrics dashboard**: Monitor your account's concurrency usage and identify when you're approaching limits. See [Observability metrics](/docs/platform/monitor/observability-metrics) for more details.

2. **Function run inspection**: View individual function runs to see step timing and identify queuing delays. See [Inspecting function runs](/docs/platform/monitor/inspecting-function-runs).

3. **Account limits**: Your plan determines your maximum concurrency limit. Check your current limits on the [billing page](https://app.inngest.com/billing) or see [pricing](/pricing?ref=docs-concurrency-troubleshooting) for plan comparisons.

**Solutions:**

1. **Increase your plan's concurrency limit**: If you're consistently hitting limits, consider upgrading your plan

2. **Optimize concurrency keys**: Use [concurrency keys](#concurrency-keys-multi-tenant-concurrency) to fairly distribute capacity across users or resources

3. **Add throttling**: If you're hitting external API rate limits, [throttling](/docs/guides/throttling) may be more appropriate than concurrency limits

4. **Use start timeouts**: Configure [start timeouts](/docs/features/inngest-functions/cancellation/cancel-on-timeouts) to cancel runs that have been waiting too long in the queue

### Understanding queuing behavior

When your concurrency limit is reached:

1. **New steps are queued**: They wait for executing steps to complete
2. **Queue order is FIFO**: Within a function, older steps are processed first
3. **No steps are dropped**: All queued steps will eventually execute (unless cancelled or timed out)
...
* Use [metrics](/docs/platform/monitor/observability-metrics) to monitor concurrency usage in production
