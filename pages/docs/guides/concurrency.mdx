...
Consider a function with a `concurrency` limit of `10`:
...
|   +------+ +------+ +------+ +------+ +------+ +------+          |
|   | [z]  | | [!]  | | [!]  | | [z]  | | [!]  | | [z]  |          |
|   |sleep | | step | | step | |wait  | | step | |sleep |          |
|   +------+ +------+ +------+ +------+ +------+ +------+          |
|                                                                  |
|   [!] = Executing (counts toward limit)     Currently: 3/3      |
|   [z] = Sleeping/Waiting (does NOT count)                       |
|                                                                  |
+------------------------------------------------------------------+
```

<Tip>
  See an animated visualization of how concurrency works on [the Inngest homepage](/?ref=docs-concurrency-guide) — look for the "Flow control" section which shows concurrency in action.
</Tip>

<Info>
  **What counts against concurrency:**
  - `step.run()` - while the step's code is executing
  - Any code executing in your function handler
...
This design allows you to have long-running workflows with many paused functions without consuming capacity. For example, a user onboarding sequence that runs over several days only consumes capacity during the brief moments when each step executes.
...
## Concurrency vs. Throttling

Understanding when to use concurrency versus [throttling](/docs/guides/throttling) is important:

| | Concurrency | Throttling |
|---|---|---|
| **What it limits** | Number of _steps_ executing simultaneously | Number of _function runs_ starting in a time period |
| **Applies to** | Step execution | Function run starts |
| **Use case** | Limit parallel workloads (DB connections, API calls) | Limit throughput over time (rate limits) |
| **Example** | "Only 10 steps can run at once" | "Only 5 functions can start per minute" |

**Use concurrency when:**
- You want to limit how many operations happen in parallel
- You need to prevent overwhelming a database or external service
- You want to ensure fair resource distribution per tenant

**Use throttling when:**
- You need to stay within a third-party API's rate limit (e.g., 100 requests/minute)
- You want to spread function execution evenly over time
- You want to control how often new work begins, regardless of how many steps run
