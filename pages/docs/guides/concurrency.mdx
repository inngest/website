...
**Concurrency is strictly ordered from oldest to newest ([FIFO](https://en.wikipedia.org/wiki/FIFO))** with ordering kept amongst individual functions.  Ordering amongst different functions is not guaranteed. so Inngest prioritizes finishing older functions above starting newer functions, even if the older functions continue to schedule new steps to run.
