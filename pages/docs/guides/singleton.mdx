import { Callout, Info, Tip, Warning, CodeGroup, Properties, Property, VersionBadge } from "src/shared/Docs/mdx";
...
[Rate Limiting](/docs/guides/rate-limiting) is similar to Singleton Functions, but it is designed to limit the number of concurrent runs of a function, whereas Singleton Functions are designed to ensure that only a single run of a function occurs over a given time window.
