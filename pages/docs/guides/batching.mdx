...
Batching breaks a lot of other primitives, therefore there are features that simply doesn't work when combined
with batching. This is true for any type of configuration that are operated on a single event because it's impossible
to tell if a condition that was valid for an event will still be true when there're now a list of them.
