	...
	func(ctx context.Context, input inngestgo.Input[map[string]any]) (any, error) {
		...
		for i, evt := range events {
			attrs[i] = map[string]interface{}{
				"user_id":   evt.Data["user_id"],
				"endpoint":  evt.Data["endpoint"],
				"timestamp": toDateTime(evt.Timestamp),
        "account_type": evt.Data["account_type"],
			}
		}
...
@inngest_client.create_function(
    ...
    batch_events=inngest.Batch(
        ...
        key="event.data.user_id",  # Optional: batch events by user ID
        if_exp="event.data.account_type == \"free\"",  # Optional: Only batch events from free accounts
    ),
)
...
You can combine batching with [concurrency](/docs/guides/concurrency) limits. For example, setting `concurrency: { limit: 1 }` will process one batch at a time.
...
* [TypeScript SDK Reference](/docs/reference/typescript/v4/functions/create#batchEvents)
