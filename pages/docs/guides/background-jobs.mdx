...
The example below shows a background job that uses an event (here called `user/created`) to send an email to new signups. It consists of two parts: creating the function that runs in the background and triggering the function.

<aside>
👉 To be able to use the code below, remember to make first [serve your functions in your Inngest API](/docs/sdk/serve) to make it available to Inngest.
</aside>

### 1. Create the function that runs in the background

<Row>
  <Col>
Let's walk through the code step by step:
1. We [create a new Inngest function](/docs/reference/functions/create), which will run in the background any time the `user/created` event is sent to Inngest.
...
Your `sendSignUpEmail` function will be triggered whenever Inngest receives an event called `user/created`. is received. You send this event to Inngest like so:
...
- Blog post: building _banger.show_'s [video rendering pipeline](/blog/banger).