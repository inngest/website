...
inngestgo.CreateFunction(
	...
	func(ctx context.Context, input inngestgo.Input[PromptCreatedEvent]) (any, error) {
		...
		imageURL, err = step.Run(ctx, "generate-image-dall-e", func(ctx context.Context) (any, error) {
			// Open API call to generate image with Dall-E...
		})
		if err != nil {
			imageURL, err = step.Run(ctx, "generate-image-midjourney", func(ctx context.Context) (any, error) {
				// MidJourney call to generate image...
			})
			via = "midjourney"
		}
