...
- Implementing [delayed code execution](/docs/learn/inngest-steps) after a specified period.
- [Debouncing events](/docs/reference/typescript/v4/functions/debounce) to minimize duplicate processing.
...
Every Inngest function handler comes with an additional [`step` object](/docs/reference/typescript/v4/functions/step-sleep-until) which provides tools to create more fine-grained functions. Using `step.run` allows you to encapsulate specific code that will be automatically retried ensuring that issues with one part of your function don't force the entire function to re-run. Additionally, [other tools like `step.sleep`](/docs/reference/typescript/v4/functions/step-sleep) are available to extend your app's functionality.
