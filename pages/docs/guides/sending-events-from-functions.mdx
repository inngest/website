...
To send events from within functions, you will use [`step.sendEvent()`](/docs/reference/functions/step-send-event). This method takes a single event, or an array of events. This example, will use an array of events.
...
Next, we create a function that listens for the `"app/weekly-email-activity.send"` event. This function will be triggered for each user that was sent an event in the previous function.
...
Another technique similar is running multiple steps in parallel ([read the guide here](/docs/guides/step-parallelism)). Here are the key differences:
