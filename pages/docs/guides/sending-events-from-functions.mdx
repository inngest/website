...
* You want to [delegate tasks to sub-agents asynchronously](/docs/ai-patterns/sub-agent-delegation#delegate-asynchronously-with-stepsendevent) in an AI agent system
* You want to [cancel](/docs/guides/cancel-running-functions) another function
* You want to send data to another function [waiting for an event](/docs/reference/typescript/v4/functions/step-wait-for-event)
...
To send events from within functions, you will use [`step.sendEvent()`](/docs/reference/typescript/v4/functions/step-send-event). This method takes a single event, or an array of events. The example below uses an array of events.
...
By using [`step.sendEvent()`](/docs/reference/typescript/v4/functions/step-send-event) Inngest's SDK can automatically add context and tracing which ties events to the current function run. If you use [`inngest.send()`](/docs/reference/typescript/v4/events/send), the context around the function run is not present.
