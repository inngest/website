...
Priority allows you to dynamically execute some runs ahead or behind others based on any data. This allows you to prioritize some jobs ahead others without the need for a separate queue. Some use cases for priority include:

* Give higher priority based on a user's subscription level, for example, free vs. paid users
* Ensure that critical work is executed before other work in the queue
* Prioritize certain jobs for users during onboarding to give the user a better first run experience
