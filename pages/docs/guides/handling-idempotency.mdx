...
To prevent an event from being handled twice, you can set a unique event `id` when [sending the event](/docs/reference/events/send#inngest-send-event-payload-event-payload-promise). This `id` acts as an idempotency key **over a 24 hour or more period** and Inngest will check to see if that event has already been received before triggering another function.
...
* Event IDs will only be used to prevent duplicate execution for a 24 hour or more period. After this period, the event will be treated as a new event and will trigger any functions with that trigger.
...
<Callout variant="warning">
  The 24 hour event idempotency period starts after the last-triggered function run for that event ID ends. For example, if you sent an event with ID `abc123` and the triggered run takes 7 days to complete, then the event idempotency period is 8 days (7 days + 24 hours).
</Callout>
...
<Callout variant="warning">
  ...
  - Debouncing
  - Event batching
  - Function pausing. When a function is paused, event idempotency is ignored. So if a replay is created after unpausing, it may have "skipped" runs that ignored event idempotency.
</Callout>
