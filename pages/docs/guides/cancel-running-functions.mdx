...
With the `POST /cancellations` endpoint, you can cancel functions by specifying the `app_id`, `function_id`, and a `from` and `to` timestamp range. You can also optionally specify an `if` statement to only cancel functions that match a [given expression](/docs/guides/writing-expressions).
...
[Read the full REST API reference here](https://api-docs.inngest.com/docs/inngest-api).