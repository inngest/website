...
- [How event-driven workflow engines work](#how-event-driven-workflow-engines-wor)
...
Here's an example of a billing function written as a workflow (using Inngest, but
...
find in an event stream.  Inngest is a durable workflow engine which allows you to also work with events.
If you're familiar with `onClick` handlers on the web, you're familiar with events.  An event is an
object that contains data which is created whenever something happens. A webhook is an event.  A user signup
is an event. And so on.
...
This improves improves scale, running functions seamlessly across a fleet of executors without worrying about
