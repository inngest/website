...
image: /assets/blog/durable-workflow-engines.jpg
date: 2023-09-28
...
about queues, state, or events altogether.
...
much simpler:  you write code as a series of steps in a single function. Each step which is enqueued
...
failure.  Each step's data is recorded in "memory", and is available for use later within the function.
...
It's also a force multiplier.  Any engineer can pick this up and work on complex sequences without
worrying about race conditions, retries, backpressure, or idempotency.  Frontend engineers,
backend engineers, and product engineers can jump right in without learning queueing specifics.
This frees up time and makes teams more effective.
...
From there, these workflow engines works as expected:  it pushes a job into a queue so that workflows
...
improves improves scale, running functions seamlessly across a fleet of executors without worrying about
