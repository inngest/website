...
Let's pin down two key points that we'll need to achieve this.

**Discoverability.** Our developer should be able to easily discover the capabilities of the code and how to use it, without referring to external sources or having to read it directly.
...
Let's dive in to an example to show how we can adapt TypeScript to protect our code far and beyond the usual inference. We'll start simple - I want to create an object that contains a list of hobbies and whether or not it's my hobby.
...
// @annotate: left { "textDegree": "3deg" } - The compiler will make sure we account for every hobby!
...
As you can see above, we declare a union type called `Hobby`, then use it when declaring `myHobbies` as `Record<Hobby, boolean>`. The TypeScript compiler can see that we're not specifying all of the hobbies we have listed and will fail to compile, protecting us from missing any options.

This is a huge boon - if I have multiple objects around my codebase using this pattern, updating the `Hobby` union will show me every other place I then need to account for the new hobby.
...
`likeDancing` is once again a `boolean`. The problem we're seeing is that const assertions can only be applied to [simple literal expressions](https://www.typescriptlang.org/docs/handbook/release-notes/typescript-3-4.html#caveats-1) that they can then infer the values of - our typed object now overrides that and widens the types back to `boolean` instead of `true` or `false`.

Removing the `Record<Hobby, boolean>` type would mean we could then add any key to the object which isn't ideal; we want to enforce the correct keys and values.
...
1. We have a safe `createHobbies` function which will enforce users to account for every hobby
...
Now we know that the TypeScript compiler can do really cool inference when prompted, let's look at how that might translate to writing a new function in your codebase to provide your developers with a guided, first-class, type-safe, and enjoyable experience
...
To start with, we could write this with nice generic types.
...
const on = (
  ...
  // ...
// @annotate: left { "textDegree": "3deg" } - We're protected at compile time, and we get autocomplete!
};
...
In our case, we also want the event to send some specific data, which changes based on the event.
...
Much better. We've made `on` a generic function that will infer its generic argument from the input. The compiler understands that `"server.start"` is the event, so can appropriately infer what the input of our function will be.

The user is still manually naming their `success` argument, though. Even though they can see it's a boolean, it might not be clear what it's representing. Finally, let's make our incoming arguments an object so that we can name them explicitly for the user.
...
Fantastic! By defining an object and encouraging destructing, it's now super easy for the user to see exactly what they're dealing with. The compiler holds their hand to push them back on track should they stray from our guided path and our explicitly naming ensures the user can discover what they have access to without leaving their editor.
...
Our user here has no idea what to type. The best case scenario is that there's some helpful JSDoc comments for the function to give them some idea of where to go, but it's highly likely that anyone writing this code (or reading it) will have to head to your docs to understand what to do.

In addition, the user is running a huge risk of runtime errors because the compiler can't tell what the data should be.

![Better user experience](/assets/blog/building-educational-typescript-tooling/on_good.gif)

Worlds apart! With our better types, our user is guided through the process without having to read anything at all! Autocomplete kicks in and shows them exactly what they can do every step of the way.

Not only is this code producing fewer errors - it's also faster to write, more enjoyable to write, and easier to read and parse further down the line.

## Conclusion

Many practices go in to making any piece of code easy to use, but baking the discovery process in to the tooling itself is often overlooked.
