...
![Without and with Event Batching](/assets/blog/announcing-batch-keys/batch-keys-1.jpg)

Developers quickly started to apply *Event Batching* to product-oriented use cases, such as “Grouping similar notifications”, resulting in added complexity in the Functions' body to partition the group events based on event properties, for example, when sending post likes notifications:
...
Based on these observations and following multiple community requests, we introduce *Batch keys* to improve the *Event Batching* experience.
...
![Event Batching vs. Event Batching with keys](/assets/blog/announcing-batch-keys/batch-keys-2.jpg)

Developers can now rely on Inngest to compose batches that better match their product needs, removing the need for custom logic and the risks of fairness issues.
