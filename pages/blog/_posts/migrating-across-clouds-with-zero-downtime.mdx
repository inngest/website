...
{/* [TODO - screenshot of GIF of this flow: https://github.com/inngest/inngest/pull/1050 when live] */}
...
async def fn(
    ...
    async def transcode_audio() -> str:
        ...
        return transcode(filename, 'aac')

    await step.run("transcode-audio", transcode_audio)

    async def generate_transcript() -> str:
        return # ... business logic omitted for sake of example ...

    await step.run("generate-transcript", generate_transcript)

    async def summarize() -> str:
        return # ...

    await step.run("summarize-with-ai", summarize)

    async def save_to_db() -> uuid.UUID:
        return # ...
