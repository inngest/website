...
showSubtitle: false
subtitle: "Lydia Hallie's powerfull animated illustrations introduce us to the inner workings of Durable Functions."
date: 2024-09-18
author: Lydia Hallie
disableCTA: false
---

Modern web applications often require **long-running tasks** that need to be **resilient to external errors and interruptions**. If such long-running tasks aren't handled properly, any error could lead to data inconsistencies, financial losses, poor user experiences, and overall system failures. 
...
Inngest's SDKS allows you to create steps through the [`step.run`](http://step.run) function. 
...
Inngest's SDKS allows you to create waits through several methods, one of which is the [`step.waitForEvent`](https://www.inngest.com/docs/reference/functions/step-wait-for-event) function.  
...
- **Advanced Error Handling**: Durable Functions are equipped with built-in mechanisms for [error detection and automatic retries](https://www.inngest.com/docs/guides/error-handling), which can be tuned to address specific recovery scenarios. This not only improves the reliability of task execution, but also reduces the manual oversight that was required with the traditional approach.
...
- The functions need to be [served through HTTP endpoints](https://www.inngest.com/docs/learn/serving-inngest-functions). In this step-by-step example, let's assume that we've successfully done this.
