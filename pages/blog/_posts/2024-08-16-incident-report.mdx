...
The SDK Gateway services log each request for audit purposes, and ran out of disk space at roughly the same time due to increased load and logrotate running too infrequently, causing the service to stall. The issue was fixed on each instance to bring them back online and prevent future failures of this type.

We apologize for incurring this downtime for all Inngest customers. System reliability is of the utmost importance for us, and we will ensure this type of failure does not happen again.

Full timeline, root cause and corrective actions are included below with additional detail.
...
![System architecture showing executors and SDK Gateway](/assets/blog/2024-08-15-incident-report/architecture.png)

For audit purposes, the SDK Gateway logs details on every request it makes.  Due to increased throughput beginning the morning of Thursday 15th, instances started running our of disk space.  The lack of disk space prevented logging, which caused the processes running on the instances to hang. Log rotation was not properly configured on these instances. Fixing this issue immediately require manual intervention. Log rotation has since been configured to run hourly on these instances.
...
Function execution was down fully for 46 minutes, degraded for 4 hours 54 minutes.
...
We apologize for incurring downtime causing issues for Inngest customers. System reliability is the most important goal for our company and we acknowledge that is the most significant outage over the last three years.

This is not something we take lightly and, in addition to the corrective actions, we will do an additional audit of potential points of failure in our system to prevent future issues. We will will continue to invest in reliability and resiliency to prevent future issues. Thank you for reading - if you have any questions on the above, please reach out to [our support team here](https://app.inngest.com/support).