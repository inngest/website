---
heading: "Global Event API: Lower latency from everywhere"
subtitle: "Global Event API beta early metrics: 46-85% reduction in response times."
showSubtitle: true
image: /assets/blog/global-event-api/feature.png
date: 2024-01-26
author: Dan Farelly
disableCTA: true
---
...
That’s why, today we are announcing our new **global Event API** as a closed beta.
...
{/* <Diagram of latency from regions?> */}
...
- **Essential [payload validation](https://www.inngest.com/docs/events#event-payload-format)**. Events that do not follow the required payload should be rejected as fast as possible
...
- **Payload size limit expansion**. We currently limit the size of payloads to 4MB, but customers often request larger sizes, especially when [sending events in bulk](https://www.inngest.com/docs/events#sending-multiple-events-at-once). While the new API will have limits to start, it will be designed for larger payload sizes.
...
You can be first in line to try the new Event API by upvoting the project [directly on our roadmap](https://roadmap.inngest.com/roadmap?id=cddffcf8-47ba-44ac-a6c3-dd318f9eb132) with your Inngest account email. Please share any context on your application that you’d like to start using it for. Additionally, if you just want to be notified when the feature comes out of beta for wide usage, [add your email here](https://www.inngest.com/newsletter?tag=beta-event-api).

We’re planning to roll this out first for sending events via [our SDKs](https://www.inngest.com/blog/cross-language-support-with-new-sdks) or [HTTP](https://www.inngest.com/docs/events#send-events-via-http-event-api). Due to the requirements for [transforms](https://www.inngest.com/docs/platform/webhooks#defining-a-transform-function), webhooks will not be part of this beta, but more webhook features will be coming later this year!
