...
We can fix that using chaining with durable execution. Every individual API call can become a separate run in a [multi-step process](https://www.inngest.com/docs/functions/multi-step?ref=blog-manage-capacity-ai). Using multi-step functions, you can:
