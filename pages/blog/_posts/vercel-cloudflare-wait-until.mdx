---
heading: When to use Vercel and Cloudflare's waitUntil utility
...
Recently, [Vercel announced](https://vercel.com/changelog/waituntil-is-now-available-for-vercel-functions) that their `waitUntil` utility method is now available for all serverless functions. Now, this useful feature is available for both Vercel's Node.js and edge functions. A similar utility is also available for [Cloudflare Workers](https://developers.cloudflare.com/workers/runtime-apis/context/#waituntil).

This is a very useful tool, but it's important to understand when to use it and when not to use it. In this post, I'll explain:
...
Long answer: `waitUntil` is not designed for running background jobs. There are no retries or tools for handling failures. For running background jobs, you should use a system that handles queueing, retries, and logs failures.
