...
Check out the [v2 migration guide](https://www.inngest.com/docs/guides/logging?ref=blog) to learn how to upgrade today.

## Better Event Typing

There were some common pain points around declaring event payload types in v1. You were always locked into a specific internal structure, but we also supported a few different inputs, meaning TypeScript errors attempting to help users get it right would be _huge_.
...
- Allows you to quickly discover every way to specify event payload types
...
Check out [Defining Event Payload Types](https://www.inngest.com/docs/reference/client/create#defining-event-payload-types) to learn about specifying types in v2.
...
Some everyday use cases for middleware include error monitoring, data transformations, logging support, and pre-run setup like connecting to databases or adding tooling or patterns using Inngest's step tooling as building blocks.
...
Check out the [Advanced: Middleware](https://www.inngest.com/docs/reference/middleware/overview/?ref=blog) documentation to get started with middleware and see some examples.
...
const fn = inngest.createFunction(
  ...
  async ({ event, step, logger }) => {
    ...
    const val = step.run("do something", () => {
...
Check out [Logging in Inngest](https://www.inngest.com/docs/guides/logging/?ref=blog) to see how to get started specifying your own logger and see how we add helpful metadata to your logs, too!
...
We're excited to see what you'll create with v2! Check out the [v2 migration guide](https://www.inngest.com/docs/guides/logging?ref=blog) to learn how to upgrade today.
