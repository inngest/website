...
- **Complex**: `async.data.seller == 'Fantastic Store' && async.data.region == 'GB' && async.data.request.operation == 'reports.getReportDocument' && async.data.request.path.reportDocumentId == 'fb.doc.2408'`
...
Our matching engine is a simple yet effective lookup for string equality matching within expressions. Here's how it works its magic:
