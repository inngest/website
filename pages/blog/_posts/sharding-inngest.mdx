---
heading: "Sharding Inngest: how we built and rolled out our new state coordinator with zero downtime"
image: /assets/blog/sharding-inngest/featured-image.png
date: 2024-07-16
author: Bruno Scheufler
disableCTA: true
---
**In June, we completed our rollout to a sharded infrastructure at Inngest, paving the way for continued growth in the months to come. As our customers use Inngest in their critical applications, we designed an effective rollout strategy to minimize risk, switching to the new infrastructure without a second of downtime. In this post, we'll walk through the decisions we made along the way, hoping that we can help engineering teams in similar situations.**

Inngest enables developers to write [durable functions](https://www.inngest.com/docs/learn/how-functions-are-executed) with [flow control](https://www.inngest.com/docs/guides/flow-control). To reliably execute long-running functions, function state is stored within Inngest to resume and retry functions without losing execution progress. This state must be stored at low latency, but persisted indefinitely. Functions might execute for seconds or months. Function state, including user-generated output, can grow over time, yet we need to ensure consistently high performance. **On an average day, we serve hundreds of millions of events.**
...
However, there's a catch: Because Inngest is a reliable layer for so many applications, we can't introduce downtime for migrations so shutting down parts of our system even for a few minutes is out of the question. So, what can we do?

Let's start by going over the previous work on the State Coordinator, which validated the system design and rollout strategy to be used in our sharding efforts.
...
Two weeks after starting at Inngest, I joined forces with Jack Williams, our founding engineer, and kicked off work on a coordinator service mediating access to the state store. The state store persists events, step outputs, and other metadata on every function run. As users can return blobs and other data from their function run steps, we need to ensure incredibly fast reads and writes to the state store to keep latency as low as possible. With the new state coordinator service, we consolidated access to the state store in a single place, pooling Redis connections for a subset of operations. If you're interested in our work on the state coordinator, please check out my recent post on the design decisions and rollout strategy we chose.

## Sharding Redis

Having rolled out the state coordinator to production without further issues, Jack and I moved our attention onto addressing the two big limitations of our previous single-cluster architecture: running out of storage capacity and blocking Redis' processing loop by handling too many queries per second.
...
Unfortunately, scaling the existing cluster was impossible without downtime. Resizing the machine would have incurred at least 20 minutes of system-wide downtime while adding another node could have caused automatic key rebalancing of critical system state, blocking the entire system for an unknown duration. Accordingly, scaling the existing cluster was off the table.
...
Reading the Redis Clustering docs gave us the inspiration for a solution that could combine the best of both worlds: **What if we could keep using our existing cluster as is, while routing new data to a multi-shard Redis cluster?** In this scenario, we could extend storage capacity to serve for the next months and beyond, while avoiding any downtime. The new cluster would run multiple nodes to distribute CPU load, and we could easily add or remove nodes over time with minimal impact.

![Sharding new data only ](/assets/blog/sharding-inngest/sharding-new-data.jpg)
...
![Strategy to selecting client and key generator](/assets/blog/sharding-inngest/decision-tree.jpg)
...
![Rollout policy stored as code](/assets/blog/sharding-inngest/rollout-policy.jpg)
...
![A graph showing recovering system](/assets/blog/sharding-inngest/recovering-system.jpg)
