---
heading: Inngest - Add Super Powers To Serverless Functions
...
**That's why we're thrilled to introduce the next version of Inngest, a developer platform that makes it easy to add superpowers to your serverless functions.** With Inngest, you can run reliable background tasks without the hassle of traditional queues or the headache of custom orchestration logic.

---

Inngest fixes all these challenges for every engineering team. It is a zero-upfront configuration system that handles orchestration and manage the state for your background tasks, from simple jobs to complex workflows. Inngest reliably calls your code via HTTP, which enables support for serverless and serverful by default. There are no queues or event streams to configure - you send events, and Inngest automatically executes your functions, managing function state and retries under the hood.

![How Inngest works](/assets/blog/inngest-announcement-april-2023/how-inngest-works.png)

Our platform includes all the table stakes features that every team needs like automatic retries, logging, event history. We've also built in simple ways to handle idempotency, manage concurrency, fan-out work to other jobs, and durably sleep. For more complex workflows, Inngest enables you to schedule work in the future and wait for additional input to pause, continue or conditionally run code.

## Developer experience

Instead of toiling over infrastructure, you can now focus on building powerful applications with just a few lines of code. We designed our **TypeScript SDK** so you can easily define functions with regular TypeScript code in your existing codebase. We want the SDK to get out of your way, not be something that takes days to understand after reading docs and guides.
