...
Today, you can handle errors with more flexibility with standard language primitives and patterns so you can combine the reliability of automatic retries with graceful error handling.
...
* Leverage maximum retries while ensuring that your …?? TODO

## Native error handling with retries

You can gracefully handle errors with the standard ways you would with [any language SDK](/blog/cross-language-support-with-new-sdks). With TypeScript, you can now wrap one or more steps within a try/catch block. Here is an example of a simple function which attempts to generate an image with DALL-E and if it fails, try to generate an image with Midjourney:
...
inngestgo.CreateFunction(
	...
	func (ctx context.Context, input inngestgo.Input[AccountConnectedEvent]) (any, error) {

	  // Perform a step, and if it fails, ensure that we clean up any partially imported data
	  data, err := step.Run(ctx, "import-data", func(ctx context.Context) (bool, error) {
		  // omitted for the sake of brevity
		  return result, err
	  })
	  if err != nil {
		  _, cleanupErr := step.Run(ctx, "cleanup-failed-import", func(ctx context.Context) (bool, error) {
			  // omitted for the sake of brevity
			  return result, err
		  })
...
This change is available today in our [TypeScript](/docs/reference/sdk/typescript) (`v3.12.0`) and [Go](/docs/reference/sdk/go) (`v0.6.0`) SDKs. We will be rolling this out to the Python SDK in the coming days. You'll also need the latest version Inngest Dev Server ([`v0.25.0`](https://github.com/inngest/inngest)) which you can run using `@latest`, for example: `npx inngest-cli@latest dev`.
