...
image: /assets/blog/adding-workflows-to-an-astro-app-with-Inngest/feature.png
date: 2024-01-12
...
If you’re familiar with both Astro and Inngest, you can read about [how to set up the Inngest API in Astro](https://www.inngest.com/docs/sdk/serve#framework-astro) or explore [the Inngest and Astro 'Hello World' example](https://github.com/inngest/inngest-js/tree/main/examples/framework-astro).
...
Astro’s most common use cases include blogs, marketing sites, and SaaS websites. To see what's possible, visit [the rich collection of themes](https://astro.build/themes/) (templates) on their website.
...
Moreover, product engineers are often responsible for long-running tasks thatwhich may require scheduling, queuing, retries, and monitoring:
...
Instead, a single engineer, product manager, or even indie hacker can use Inngest to handle the execution but also see the [error logs](https://www.inngest.com/docs/guides/logging), have the [failed tasks be automatically retried](https://www.inngest.com/docs/functions/retries), and obtain useful metadata.
...
You can also [send an event](https://www.inngest.com/docs/events) from an Astro site that triggers an Inngest function in *another* codebase.

In other words, a function owned by the product team can live in the Astro site and other functions in your backend codebase. At the moment, Inngest has three SDKs available ([TypeScript/JavaScript](https://www.inngest.com/docs/reference/typescript), [Python](https://www.inngest.com/docs/reference/python), [Go](https://pkg.go.dev/github.com/inngest/inngestgo)) but [others are planned as well](https://pkg.go.dev/github.com/inngest/inngestgo).

## Get started now

If you'd like to learn more about Inngest, head to our [Quick Start Guide](https://www.inngest.com/docs/quick-start) for a guided tour around the [Inngest Dev Server](https://www.inngest.com/docs/local-development#inngest-dev-server).
...
You will notice that [Inngest functions](https://www.inngest.com/docs/functions) are available at `src/inngest` and the [Inngest handler](https://www.inngest.com/docs/sdk/serve) is available a `src/pages/api/inngest.ts`.

---

Do you have any questions about Astro+Inngest? Or ideas for apps you wanna build? Join us our Inngest community on [Discord](https://www.inngest.com/discord) or reach out on [Twitter](https://twitter.com/inngest).
